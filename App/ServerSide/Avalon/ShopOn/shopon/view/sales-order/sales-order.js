$.fn.salesOrder = function () {
    return $.pageController.getPage(this, function (page, $$) {
        ...
        page.eventsAPI = new EventsAPI();
        page.salesOrderAPI = new SalesOrderAPI();
        page.billPaymentAPI = new BillPaymentAPI();
        page.finfactsEntryAPI = new FinfactsEntryAPI();
        page.returnOrderAPI = new ReturnOrderAPI();
        page.returnOrderItemAPI = new ReturnOrderItemAPI();
        ...
        page.newCustno = null;
        page.cancelso = false;
        page.allowReturnPayment = true;
        page.fullfillMode = false;
        page.deliveryMode = false;
        page.trayMode = false;
        page.removeItemsInGrid = false;
        page.customerList = [];
        page.searchAttributes = "text";
        page.attr_list = [];
        page.printInvoiceCheck = "button";
        page.event_order_id = "";
        //page.tax = [];
        CONTEXT.ShowMore = false;

        document.title = "ShopOn - Sales Order";
        ...
        $("[controlid=pnlNewSOPopup]").keydown(function (e) {
            var keyCode = e.keyCode || e.which;
            if (e.altKey && e.which == 78) {
                e.preventDefault();
                page.events.btnAddNewCustomer_click();
            }
        });
        $("[controlid=lblSOOrderedDate]").datepicker({
            onSelect: function () {
                if ($$("lblSOOrderedDate").getDate() != "") {
                    $("[controlid=dsSOExpectedDate]").datepicker("option", "minDate", new Date(dbDate($$("lblSOOrderedDate").getDate())));
                } else {
                    $("[controlid=dsSOExpectedDate]").datepicker("option", "minDate", new Date());
                }
            }
        });

        setTimeout(function () {
            $("[controlid=dsExpectedDate]").datepicker("option", "minDate", new Date());
        }, 100)

        page.events.btnBack_click = function () {
            if (page.selectedSO.state_text == "Created") {
                page.controls.pnlConformClosing.open();
                page.controls.pnlConformClosing.title("Confirmation Panel");
                page.controls.pnlConformClosing.rlabel("Confirmation Panel");
                page.controls.pnlConformClosing.width(550);
                page.controls.pnlConformClosing.height(220);
                $$("btnClosePopup").focus();
            }
            else {
                $$("pnlDetail").hide();
        ...
        page.events.btnConfirmPopup_click = function () {
            page.controls.pnlConformClosing.close();
            $$("pnlDetail").hide();
            $$("pnlSearch").show();
            page.events.btnSearch_click();
        }
        page.events.btnClosePopup_click = function () {
            page.controls.pnlConformClosing.close();
        }

        page.events.btnAddNewCustomer_click = function () {
        ...
        page.events.btnSaveCustomer_click = function () {
            page.controls.ucCustomerEdit.save(function (data1) {
                page.customerService.getActiveCustomer("", function (data) {
                    $$("ddlSOCustomer").dataBind(data, "cust_no", "cust_name");
                    page.customerAPI.searchData("", "", "cus_active=1", "", function (data) {
                        page.customerList = data;
                        page.controls.ddlCustomerName.selectedText(data1.first_name + " " + data1.last_name);
                        page.newCustno = data1.cust_no;
        ...
        page.events.btnAddSOOK_click = function () {
            ...
            try {
                ...
                page.customerService.getCustomerById(page.newCustno, function (data) {
                    var id = {
                        table_name: "sal_order_t",
                        column_name: "order_no"
                    }
                    page.maximumIdAPI.getStoreValue(id, function (id) {
                    
                        //Build the data for new Sales order to be created.Get sales tax from settings
                        data[0].phone_no = (data[0].phone_no == null || data[0].phone_no == "") ? "" : data[0].phone_no;
                        data[0].address1 = (data[0].address1 == null || data[0].address1 == "") ? "" : data[0].address1;
                        data[0].address2 = (data[0].address2 == null || data[0].address2 == "") ? "" : data[0].address2;
                        data[0].city = (data[0].city == null || data[0].city == "") ? "" : data[0].city;
                        data[0].zip_code = (data[0].zip_code == null || data[0].zip_code == "") ? "" : data[0].zip_code;
                        data[0].email = (data[0].email == null || data[0].email == "") ? "" : data[0].email;
                        data[0].company = (data[0].company == null || data[0].company == "") ? "" : data[0].company;
                        var newSO = {
                            cust_no: page.newCustno,//$$("ddlCustomerName").selectedValue(),
                            expected_date: dbDateTime($$("dsExpectedDate").getDate()),
                        ...
                        page.salesOrderAPI.postValue(newSO, function (data) {
        ...
        page.events.btnRecreateSO_click = function () {
            try {
                if (confirm("If You Recreate The Order The Existing Bill Is Deleted With All The Items And Payment The New Bill Is Generated, Are You Sure To Continue?")) {
        ...
        page.saveSalesOrder = function (callback) {
            ...
            $(page.controls.grdSOItems.allData()).each(function (i, item) {
                ...
                if (isNaN(item.free_qty) || parseFloat(item.free_qty) < 0)
                    item.free_qty = 0;
                if ((parseFloat(item.quantity) + parseFloat(item.free_qty)) <= 0)
            ...
            if (page.selectedSO.state_text == "Created") {
                ...
                //Order date details
                newSO.expected_date = dbDateTime(page.controls.dsSOExpectedDate.getDate());
                newSO.ordered_date = dbDateTime(page.controls.lblSOOrderedDate.getDate());
                ...
                page.salesOrderAPI.putValue(page.selectedSO.order_id, newSO, function (temp) {
                    ...
                    page.salesService.orderSalesOrder(page.selectedSO.order_id, page.controls.lblSOOrderedDate.getDate(), function (data2, callback) {
                        ...
                        $$("msgPanel").show("Generating a new bill...");
                            ...
                            $(page.controls.grdSOItems.allData()).each(function (i, billItem) {
                                billItem.total_price = (isNaN(billItem.total_price)) ? "0" : billItem.total_price;
                                rbillItems.push({
                                    sl_no: i + 1,
                            ...
                            page.stockAPI.insertBill(newBill, function (data) {
                                page.currentBillNo = data.bill_no;
                                ...
                                page.salesService.updateBillNo({ order_id: page.selectedSO.order_id, bill_no: page.currentBillNo }, function () {
                                    ...
                                    page.loadSalesOrder(page.selectedSO.order_id, function () {
                                        $$("msgPanel").flash("Sales order is successfully ordered. A new bill is generated");
                                    });
        ...
        page.events.btnConfirmedSO_click = function () {
            try {
                ...
                page.saveSalesOrder(function () {
                    ...
                    $$("msgPanel").show("Changing sales order to confirm state...");
                    ...
                    page.salesService.confirmedSalesOrder(page.selectedSO.order_id, page.controls.lblSOConfirmedDate.getDate(), function () {
                        ...
                        page.billService.changeBillType(newBill, function (data) {
                            ...
                            page.loadSalesOrder(page.selectedSO.order_id, function () {
                                ...
                                $$("msgPanel").flash("Sales order is successfully confirmed. A new bill is generated");
                        ...
                        if (CONTEXT.ENABLE_FINFACTS_MODULES == true) {
                            ...
                            var data1 = {
                                ...
                                description: "Sales Order - " + $$("lblTempSONo").html(),//page.currentBillNo,
                            ...
                            page.finfactsEntry.creditSales(data1, function (response) {
                                if (CONTEXT.ENABLE_BILL_EXPENSE_MODULES) {
                                    var expenseData1 = {
                                        ...
                                        description: "Sales Order Expense-" + $$("lblTempSOIBillNo").html(),//page.currentBillNo,
                                        jrn_date: $.datepicker.formatDate("dd-mm-yy", new Date()),
                                        comp_id: localStorage.getItem("user_finfacts_comp_id"),
                                        key_1: page.currentBillNo,
                                        key_2: $$("ddlSOCustomer").selectedValue(),
                            ...
                            page.customerService.getCustomerById(page.controls.ddlSOCustomer.selectedValue(), function (data) {
                                page.rewardService.getRewardById(data[0].reward_plan_id, function (data2) {
                                    if (data2.length != 0) {
                                        ...
                                        newItem.description = "Bill No:" + $$("lblTempSOIBillNo").html();
        ...
        page.events.btnCompleteFullfill_click = function (free) {
            if (confirm("Are You Sure Want To Fulfill The Items")) {
                ...
                try {
                    $(page.controls.grdSOItems.allData()).each(function (index, item) {
                        ...
                        if (item.free_qty == "" || item.free_qty == null || item.free_qty == undefined)
                            item.free_qty = 0;
                        item.temp_free_qty = (isNaN(item.temp_free_qty))?0:item.temp_free_qty;
                        item.tofullfilled = (parseFloat(item.temp_qty) + parseFloat(item.temp_free_qty)) - parseFloat(item.fullfilled);
                        ...
                        if (item.tofullfilled > 0) {
                            ...
                            inventItems.push({
                                ...
                                key2: item.item_no,
                                key3: page.controls.lblTempSOIBillNo.html(),
                                //FINFACTS ENTRY DATA
                                ...
                                tray_received: item.tofullfilled,
                                tray_id: item.tray_no,
                                tray_mode: item.tray_mode,//"SKU",
                    ...
                    page.stockAPI.insertStock(newBill, function (data) {
                        page.salesService.getSOItems(page.selectedSO.order_id, function (data) {
                            ...
                            page.events.btnFulfillSO_click();
                        });
                    });
        ...
        page.events.btnCompleteDelivery_click = function () {
            if (confirm("Are You Sure Want To Delivery The Items")) {
                var items = [];
                var flag = false;
                var readyForDeliverySMS = false;
                try {
                    //  var count = 0;
                    //  var item_name;
                    page.readyForDelItems = "";
                    $(page.controls.grdSOItems.allData()).each(function (index, item) {
                        ...
                        item.temp_free_qty = (isNaN(item.temp_free_qty))?0:item.temp_free_qty;
                        item.todelivered = (parseFloat(item.temp_qty) + parseFloat(item.temp_free_qty)) - parseFloat(item.delivered);
                        ...
                        if (item.todelivered > 0) {
                            ...
                            var tot_deliverable = parseFloat(item.delivered) + parseFloat(item.todelivered);
                            if (parseFloat(tot_deliverable) == parseFloat(item.fullfilled) && parseFloat(tot_deliverable) == parseFloat(item.quantity)) {
                                readyForDeliverySMS = true;
                                page.readyForDelItems = page.readyForDelItems + item.item_name + " (" + parseInt(item.quantity) + ") ,";
                            }
                        }
                        items.push({
                            order_item_id: item.order_item_id,
                    ...
                    page.salesService.updateSalesOrderItems(0, items, function () {
                        ...
                        page.salesService.getSOItems(page.selectedSO.order_id, function (data) {
                            ...
                            page.events.btnDeliveredSO_click();
                            $$("msgPanel").flash("Sales order is successfully delivered...!");
                            if (readyForDeliverySMS) {
                                var recNo = "";
                                if ($$("txtContactNo").value().indexOf(CONTEXT.SMS_COUNTRY_CODE) > -1) {
                                    recNo = $$("txtContactNo").value();
                                } else {
                                    recNo = CONTEXT.SMS_COUNTRY_CODE + "" + $$("txtContactNo").value();
                                }
                                var ajaxInfo = {
                                    "appName": CONTEXT.SMS_APPNAME,
                                    "senderNumber": CONTEXT.SMS_SENDER_NO,
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": recNo,
                                    "message": " Hi! Your " + CONTEXT.SMS_APPNAME + " Order No " + $$("lblSORefNo").html() + " for " + page.readyForDelItems + " is out for delivery.",
                                };
                                if (CONTEXT.ENABLE_SMS_TO_CUST_OUT_FOR_DELIVERY) {
                                    ...
                                    console.log("SMS Service is blocked. Please contact your admin");
                                }
                                ...
                                var ajaxInfo = {
                                    ...
                                    "senderNumber": CONTEXT.SMS_SENDER_NO,
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": CONTEXT.SMS_RECEIVER_NO,
                                    "message": CONTEXT.SMS_APPNAME + " Order No " + $$("lblSORefNo").html() + " for  " + page.readyForDelItems + " is out for delivery.",
                                };
                                if (CONTEXT.ENABLE_SMS_TO_ADMIN_OUT_FOR_DELIVERY) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                                    }, function (errorResponse) {
                                        console.log("Error while sending SMS : " + errorResponse);
        ...
        page.events.btnFulfillSO_click = function () {
            try {
                //   var count = 0;
                $(page.controls.grdSOItems.allData()).each(function (i, item) {
                    var ori_fulfill_qty = item.fullfilled;
                    if (item.unit_identity == "1")
                        ori_fulfill_qty = parseFloat(item.fullfilled) * parseFloat(item.alter_unit_fact);
                    if (item.free_qty == "" || item.free_qty == null || item.free_qty == undefined)
                        item.free_qty = 0;
                    if ((parseFloat(item.free_qty) + parseFloat(item.quantity)) != parseFloat(ori_fulfill_qty))
        ...
        page.events.btnDeliveredSO_click = function () {
            try {
                ...
                page.items = "";
                $(page.controls.grdSOItems.allData()).each(function (i, item) {
                    var ori_delivery_qty = item.delivered;
                    if (item.unit_identity == "1")
                        ori_delivery_qty = parseFloat(item.delivered) * parseFloat(item.alter_unit_fact);

                    page.items = page.items + item.item_name + " (" + parseInt(item.quantity) + ") ,";
                    if (item.free_qty == null)
                        item.free_qty = 0;

                    if ((parseFloat(item.free_qty) + parseFloat(item.quantity)) != parseFloat(ori_delivery_qty))
                ...
                page.salesService.deliverSalesOrder(page.selectedSO.order_id, page.controls.lblSODeliveredDate.getDate(), function (data2, callback) {
                    ...
                    page.loadSalesOrder(page.selectedSO.order_id, function () {
                        ...
                        var ajaxInfo = {
                            ...
                            "companyId": CONTEXT.SMS_COMPANY_ID,
                            "receiverNumber": recNo,
                            "message": " Hi! Your " + CONTEXT.SMS_APPNAME + " Order No " + $$("lblSORefNo").html() + " for " + page.items + " has been successfully delivered .Thanks for shopping with us",
                        };
                        if (CONTEXT.ENABLE_SMS_TO_CUST_AFTER_DELIVERY) {
                            sendSMS(ajaxInfo, function (response) {
                                console.log("Order SMS has been sent..!");
                            }, function (errorResponse) {
                                console.log("Error while sending SMS : " + errorResponse);
                        ...
                        var ajaxInfo = {
                            ...
                            "companyId": CONTEXT.SMS_COMPANY_ID,
                            "receiverNumber": CONTEXT.SMS_RECEIVER_NO,
                            "message": CONTEXT.SMS_APPNAME + " Order No " + $$("lblSORefNo").html() + " for  " + page.items + " has been delivered to Client successfully.",
                        };
                        if (CONTEXT.ENABLE_SMS_TO_ADMIN_AFTER_DELIVERY) {
                            sendSMS(ajaxInfo, function (response) {
                                console.log("Order SMS has been sent..!");
                            }, function (errorResponse) {
                                console.log("Error while sending SMS : " + errorResponse);
            ...
            try {
                ...
                if (confirm("Are you sure to pay the Amount") == true) {
                    ...
                    if (isNaN(parseFloat($$("txtPayAmount").val()))) {
                        ...
                        var allBillSO = [];
                        var data1 = [], data2 = [];
                        var total_paid_amount = parseFloat($$("txtPayAmount").val());
                        $(page.controls.grdBillTransactions.allData()).each(function (i, item) {
                            if (parseFloat(total_paid_amount) > 0) {
                                if (parseFloat(item.balance) > 0) {
                                    if (item.bill_type == "SaleReturn") {
                                        allBillSO.push({
                                            collector_id: localStorage.getItem("user_register_id"),//$$("ddlCollectorName").selectedValue(),
                                            pay_desc: "Sales Order Bill Payment",
                                            amount: item.balance,
                                            bill_no: item.bill_no,
                                            bill_type:item.bill_type,
                                            pay_date: dbDateTime(page.controls.dsBillPayDate.getDate()),
                                            pay_type: item.bill_type,
                                            pay_mode: page.controls.ddlPaymentBillType.selectedValue(),
                                            card_type: "",
                                            card_no: "",
                                            coupon_no: "",
                                            cheque_no: "",
                                        ...
                                        data2.push({
                                            ...
                                            paid_amount: item.balance,
                                            description: "Sales Order Bill Return-" + item.bill_code,
                                            jrn_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                                            key_1: item.bill_no,
                                            key_2: page.selectedSO.cust_no,
                                            comp_id: localStorage.getItem("user_finfacts_comp_id"),
                                        });
                                    }
                                }
                        ...
                        var data = {
                            receive_amount: parseFloat(total_paid_amount) + parseFloat($$("lblReturnsBalance").value()),
                            bill_amount: parseFloat(total_paid_amount) + parseFloat($$("lblReturnsBalance").value()),
                        ...
                        if (parseFloat(total_paid_amount) > 0) {
                            allBillSO1.push({
                                collector_id: localStorage.getItem("user_register_id"),//$$("ddlCollectorName").selectedValue(),
                                pay_desc: "Sales Order Bill Payment",
                                amount: parseFloat(total_paid_amount)+parseFloat($$("lblReturnsBalance").value()),
                            ...
                            data1.push({
                                per_id: CONTEXT.FINFACTS_CURRENT_PERIOD,
                                target_acc_id: ($$("ddlPayMode").selectedValue() == "Cash") ? CONTEXT.FINFACTS_SALES_DEF_CASH_ACCOUNT : ($$("ddlPayMode").selectedValue() == "Card") ? CONTEXT.FINFACTS_SALES_DEF_BANK_ACCOUNT : ($$("ddlPayMode").selectedValue() == "Coupon") ? CONTEXT.FINFACTS_SALES_DEF_CASH_ACCOUNTCoupon : ($$("ddlPayMode").selectedValue() == "Reward") ? CONTEXT.FINFACTS_SALES_DEF_CASH_ACCOUNTReward : CONTEXT.FINFACTS_SALES_DEF_CASH_ACCOUNT,
                                paid_amount: parseFloat(total_paid_amount) + parseFloat($$("lblReturnsBalance").value()),
                                description: "Sales Order Bill - " + $$("lblTempSOIBillNo").html(),
                                jrn_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                ...
                try {
                    ...
                    if (confirm("Are you sure to pay the Amount") == true) {
                        ...
                        if (isNaN(parseFloat($$("txtReturnPayAmount").val())) || parseFloat($$("txtReturnPayAmount").val()) <= 0) {
                            ...
                            //Prepare for invoice payment
                            var billSO = {
                                collector_id: localStorage.getItem("user_register_id"),//page.controls.ddlReturnCollectorName.selectedValue(),
                                ...
                                pay_mode: page.controls.ddlReturnPaymentBillType.selectedValue()
                            };
                            $$("msgPanel").show("Updating Payments...");
                            //Pay for the correstponding bill_no
                            page.salesService.payInvoiceBillSalesOrder(billSO, function (data) {
                                var payment_id = data[0].key_value;
                                //If finfacts module is enabled then make entry in finfacts.
                                if (CONTEXT.ENABLE_FINFACTS_MODULES == true) {
                                    var data1 = {
                                        per_id: CONTEXT.FINFACTS_CURRENT_PERIOD,
                                        ...
                                        paid_amount: parseFloat(page.controls.txtReturnPayAmount.value()).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                                        description: "Sales Order Bill Return-" + page.controls.ddlReturnBillNo.selectedData().bill_code,
                                        jrn_date: $.datepicker.formatDate("dd-mm-yy", new Date()),
                                    ...
                                    //page.finfactsService.insertCreditReturnReceipt(data1, function (response) {
                                        ...
                                        page.loadSalesOrder(page.selectedSO.order_id, function () {
                                            ...
                                            var ajaxInfo = {
                                                ...
                                                "companyId": CONTEXT.SMS_COMPANY_ID,
                                                "receiverNumber": recNo,
                                                "message": "Amount Rs." + $$("txtReturnPayAmount").val() + " is refunded for your Order No " + $$("lblSORefNo").html() + ".",
                                            };
                                            if (CONTEXT.ENABLE_SMS_TO_CUST_AFTER_CANCEL) {
                                                sendSMS(ajaxInfo, function (response) {
                                                    console.log("Order SMS has been sent..!");
                                                }, function (errorResponse) {
                                                    console.log("Error while sending SMS : " + errorResponse);
                                            ...
                                            var ajaxInfo = {
                                                ...
                                                "companyId": CONTEXT.SMS_COMPANY_ID,
                                                "receiverNumber": CONTEXT.SMS_RECEIVER_NO,
                                                "message": "Amount Rs." + $$("txtReturnPayAmount").val() + " is refunded for Order No " + $$("lblSORefNo").html() + ".",
                                            };
                                            if (CONTEXT.ENABLE_SMS_TO_ADMIN_AFTER_CANCEL) {
                                                sendSMS(ajaxInfo, function (response) {
                                                    console.log("Order SMS has been sent..!");
                                                }, function (errorResponse) {
                                                    console.log("Error while sending SMS : " + errorResponse);
        ...
        page.events.btnCancelSO_click = function () {
            if (confirm("Are you sure want to cancel the Sales Order!")) {
                page.cancelso = true;
                page.allowReturnPayment = false;
                //var temp_order_id = (page.selectedSO == null) ? "" : page.selectedSO.order_id;
                //var temp_bill_no = (page.selectedSO == null) ? "" : page.selectedSO.bill_no;
                //if (typeof order_id != "undefined") {
                //    temp_order_id = order_id;
                //    page.event_order_id = temp_order_id;
                //}
                //if (typeof bill_no != "undefined")
                //    temp_bill_no = bill_no;
                $$("txtSOIReturnBillDate").setDate($.datepicker.formatDate("mm-dd-yy", new Date()));
                page.billService.getReturnedBillByNo(page.selectedSO.order_id, page.selectedSO.bill_no, function (rdata) {
                    var tot_qty = 0, tot_return = 0;
                    $(rdata).each(function (i, item) {
                        tot_return = parseFloat(item.qty);
                        if (parseFloat(item.ordered_qty) >= parseFloat(tot_return)) {
                            item.ret_qty = item.qty;
                            item.ret_free = 0;
                        }
                        else {
                            if (parseFloat(tot_return) > parseFloat(item.ordered_qty))
                                item.ret_qty = 0;
                            tot_return = parseFloat(tot_return) - parseFloat(item.ordered_qty);
                            item.ret_free = item.qty;
                        }
                    });
                    page.events.btnReturnSOItemPopup_click(rdata, function (temp_data) {
                        page.salesService.cancelSalesOrder(page.selectedSO.order_id, function (data1) {
                            ...
                            page.items = "";
                            page.salesService.getSOItems(page.selectedSO.order_id, function (item_data) {
                                $(item_data).each(function (i, item) {
                                    if (page.items == "")
                                        page.items = page.items + item.item_name + " (" + parseInt(item.quantity) + ") ,";
                                    else
                                        page.items = page.items + "," + item.item_name + " (" + parseInt(item.quantity) + ") ,";
                                });
                                if (!CONTEXT.ESHOPON_CANCEL) {
                                    page.loadSalesOrder(page.selectedSO.order_id, function () {
                                    });
                                }
                                var recNo = "";
                                ...
                                var ajaxInfo = {
                                    ...
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": recNo,
                                    "message": "Dear " + $$("ddlSOCustomer").selectedData().cust_name + " \n Your Order No " + page.selectedSO.order_ref_no + " for " + page.items + " is cancelled.",//
                                };
                                if (CONTEXT.ENABLE_SMS_TO_CUST_AFTER_CANCEL) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                                ...
                                var ajaxInfo = {
                                    ...
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": CONTEXT.SMS_RECEIVER_NO,
                                    "message": CONTEXT.SMS_APPNAME + " Order No " + page.selectedSO.order_ref_no + " for " + page.items + " is cancelled",//" + page.items + "
                                };
                                if (CONTEXT.ENABLE_SMS_TO_ADMIN_AFTER_CANCEL) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                                ...
                                var event_filter = "order_id like '" + page.selectedSO.order_ref_no + "' and event_name like 'Order Cancel'";
                                page.eventsAPI.searchValue("", "", event_filter, "", function (data) {
                                    if (data.length != 0) {
                                        if (typeof data[0].event_id != "undefined" && data[0].event_id != null && data[0].event_id != "") {
                                            var evntData = {
                                                event_id: data[0].event_id,
                                                state_no: (data[0].state_no == "10" ? "30" : "10")
                                            }
                                            page.eventsAPI.putValue(evntData, function (data) {
                                            });
                                        }
                                    }
                                });
                                //if (CONTEXT.ESHOPON_CANCEL) {
                                //    if (typeof page.controls.grdSOEvents.selectedData()[0] != "undefined" && page.controls.grdSOEvents.selectedData()[0] != null && page.controls.grdSOEvents.selectedData()[0] != "") {
                                //        var evntData = {
                                //            event_id: page.controls.grdSOEvents.selectedData()[0].event_id,
                                //            state_no: (page.controls.grdSOEvents.selectedData()[0].state_no == "10" ? "30" : "10")
                                //        }
                                //        page.eventsAPI.putValue(evntData, function (data) {
                                //        });
                                //    }
                                //}
                                page.view.viewSOEvents();
                                page.events.btnSearch_click();
                                page.allowReturnPayment = true;
                            });
                        });
        ...
        page.events.btnCompleteReturn_click = function () {
            if (confirm("Are you sure want to complete return this Sales Order!")) {
                page.cancelso = false;
                page.allowReturnPayment = false;
                //var temp_order_id = (page.selectedSO == null) ? "" : page.selectedSO.order_id;
                //var temp_bill_no = (page.selectedSO == null) ? "" : page.selectedSO.bill_no;
                ...
                $$("txtSOIReturnBillDate").setDate($.datepicker.formatDate("mm-dd-yy", new Date()));
                page.billService.getReturnedBillByNo(page.selectedSO.order_id, page.selectedSO.bill_no, function (rdata) {
                    var lastsku_no = null,temp_data=[];
                    $(rdata).each(function (i, item) {
                        if (item.sku_no != lastsku_no) {
                            item.free_qty = (item.free_qty == null || item.free_qty == "") ? 0 : item.free_qty;
                            item.free_qty_SaleReturn = (item.free_qty_SaleReturn == null || item.free_qty_SaleReturn == "") ? 0 : item.free_qty_SaleReturn;
                            item.ret_qty = parseFloat(item.ordered_qty) - (parseFloat(item.qty_SaleReturned) - parseFloat(item.free_qty_SaleReturn));
                            item.ret_free = parseFloat(item.free_qty) - parseFloat(item.free_qty_SaleReturn);
                            temp_data.push(item);
                            lastsku_no = item.sku_no;
                        }
                    });
                    page.events.btnReturnSOItemPopup_click(temp_data, function (temp_data) {
                        page.salesService.cancelSalesOrder(page.selectedSO.order_id, function (data1) {
                            $$("msgPanel").show("Order has been returned successfully...");
                            page.items = "";
                            page.salesService.getSOItems(page.selectedSO.order_id, function (item_data) {
                                $(item_data).each(function (i, item) {
                                    if (page.items == "")
                                ...
                                var ajaxInfo = {
                                    ...
                                    "senderNumber": CONTEXT.SMS_SENDER_NO,
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": $$("txtContactNo").value(),
                                    "message": "Dear " + $$("ddlSOCustomer").selectedData().cust_name + " \n Your Order No " + page.selectedSO.order_ref_no + " for " + page.items + " is returned.",//
                                };
                                if (CONTEXT.ENABLE_SMS_TO_CUST_AFTER_CANCEL) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                                ...
                                var ajaxInfo = {
                                    ...
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": CONTEXT.SMS_RECEIVER_NO,
                                    "message": CONTEXT.SMS_APPNAME + " Order No " + page.selectedSO.order_ref_no + " for item " + page.items + " is returned",//
                                };
                                if (CONTEXT.ENABLE_SMS_TO_ADMIN_AFTER_CANCEL) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                                ...
                                var event_filter = "order_id like '" + page.selectedSO.order_ref_no + "' and event_name like 'Order Return'";
                                page.eventsAPI.searchValue("", "", event_filter, "", function (data) {
                                    if (data.length != 0) {
                                        if (typeof data[0].event_id != "undefined" && data[0].event_id != null && data[0].event_id != "") {
                                            var evntData = {
            ...
            try {
                if (page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered") {
                    ...
                    if (confirm("Are you sure want to delete the Sales Order!")) {
                        ...
                        page.salesOrderAPI.putValue(page.selectedSO.order_id, data, function (data) {
                            $$("msgPanel").show("Sales Order is moved to removed state");
                            page.events.page_load();
                            page.events.btnSearch_click();
                        }, function (data) {
                            $$("msgPanel").show("Some Internal Problems Will Occur, Please Try Again Later...!")
                        })
        ...
        //Receipt Print
        page.events.btnPrintSOReceipt_click = function (bill_no) {
            //if (CONTEXT.SALESORDER_RECEIPT_TEMPLATE == "Template1") {
                page.events.btnPrintReceiptTemplate(page.selectedSO.bill_no);
            //}
            //if (CONTEXT.SALESORDER_RECEIPT_TEMPLATE == "Template2") {
            //    page.events.btnPrintReceiptTemplate1(bill_no);
            //}
        }
        page.events.btnPrintRequestForFulfill_click = function () {
            try {
                if (page.selectedSO.bill_no == "" || page.selectedSO.bill_no == null || typeof page.selectedSO.bill_no == "undefined") {
                    throw "Bill Is Not Generated Yet...!";
                }
                page.events.btnPrintReceiptTemplate1(page.selectedSO.order_id);
            }
            catch (e) {
                alert(e);
            }
        }
        page.events.btnPrintReceiptTemplate = function (bill_no) {
            ...
            page.billAPI.getValue(bill_no, function (data) {
                ...
                $(data.bill_items).each(function (i, item) {
                    ...
                    billData.push({
                        ...
                        Amount: parseFloat(item.total_price).toFixed(CONTEXT.COUNT_AFTER_POINTS),//parseFloat(parseFloat(item.price) * parseFloat(temp_qty)).toFixed(2),
                        Discount: convertBitString(item.discount, 9),

                        ProductData1: i + 1,
                        ProductData2: item.item_name,
                        ProductData3: convertBitString(item.qty, 16),
                        ProductData4:convertBitString(temp_price.toString(), 12) + " ",
                        ProductData5: "",//convertBitString(item.discount, 8),
                        ProductData6: parseFloat(item.total_price).toFixed(CONTEXT.COUNT_AFTER_POINTS),//parseFloat(parseFloat(item.price) * parseFloat(temp_qty)).toFixed(2),
                    });
                });
                if (data.state_text == "Return") {
                ...
                var billSummary = "Sale + Tax:" + parseFloat(subTotal).toFixed(CONTEXT.COUNT_AFTER_POINTS);// + " | Dis:" + parseFloat(data.discount).toFixed(2);
                var printData = {
                    ...
                    BillType: bill_title,
                    //BillNo: bill_no,
                    BillNo: "Order No : " + page.selectedSO.order_no,
                    ...
                    BarCodeText: getBitByZero(page.selectedSO.order_no, 10),
                    Template: CONTEXT.RECEIPT_TEMPLATE,
                    ProductHeading1: "Sl No",
                    ProductHeading2: "Product",
                    ProductHeading3: convertBitString("Qty", 16),
                    ProductHeading4: convertBitString("Rate", 12),
                    ProductHeading5: "",//convertBitString("Rate", 8),
        ...
        page.events.btnPrintReceiptTemplate1 = function (order_id) {
            $$("msgPanel").flash("Printing the selected Bill.Please wait.");
            var date = new Date();
            var hours = date.getHours();
            var minutes = date.getMinutes();
            ...
            var billData = [];
            var tot_qty = 0;
            var print_bill=false;

            page.salesService.getSOByNo(order_id, function (data) {
                data = data[0];
                page.salesService.getSOItems(order_id, function (data_item) {
                    //page.billAPI.getValue(bill_no, function (data) {
                    var bill_title = "Request for Fulfillment";
                    var subTotal = 0;
                    $(data_item).each(function (i, item) {
                        if (parseInt(item.send_to_fulfill) != (parseInt(item.quantity) + parseInt(item.free_qty))) {
                            print_bill = true;
                            var temp_qty = item.quantity;
                            var temp_price = item.price;
                            subTotal = subTotal + parseFloat(temp_price) * parseFloat(temp_qty);
                            if (parseInt(item.free_qty) != 0) {
                                //tot_qty = tot_qty + parseInt(item.quantity) + parseInt(item.free_qty);
                                item.quantity = parseInt(parseInt(item.quantity) + parseInt(item.free_qty));
                            }
                            else {
                                //tot_qty = tot_qty + parseInt(item.quantity);
                                item.quantity = parseInt(item.quantity);
                            }
                            //if (data.state_text != "Return")
                            //    if (item.tax_inclusive == "1")
                            //        item.price = parseFloat(parseFloat(item.price) / parseFloat((parseFloat(item.tax_per) / 100) + 1)).toFixed(2);
                            item.quantity = parseInt(item.quantity) - parseInt(item.send_to_fulfill);
                            tot_qty = tot_qty + parseInt(item.quantity);
                            item.quantity = (parseInt(item.quantity)) + " " + item.unit;
                            billData.push({
                                SlNo: i + 1,
                                ItemName: item.item_name,
                                Rate: convertBitString(temp_price.toString(), 6) + " ",
                                Price: convertBitString(item.price.toString(), 6),
                                Qty: convertBitString(item.quantity, 12),
                                Amount: parseFloat(item.total_price).toFixed(CONTEXT.COUNT_AFTER_POINTS),//parseFloat(parseFloat(item.price) * parseFloat(temp_qty)).toFixed(2),
                                Discount: convertBitString(item.discount, 9),

                                ProductData1: convertBitString("" + (i + 1) + "", 2) + " " + convertBitString(item.item_name, 26) + " " + item.quantity + "",
                                ProductData2: "",
                                ProductData3: "",
                                ProductData4: " ",
                                ProductData5: "",//convertBitString(item.discount, 8),
                                ProductData6: "",//parseFloat(parseFloat(item.price) * parseFloat(temp_qty)).toFixed(2),
                            });
                        }
                    });
                    //if (data.state_text == "Return") {
                    ...
                    //}
                    //var billSummary = "Sale:" + parseFloat(subTotal).toFixed(2) + " | GST:" + parseFloat(data.tax).toFixed(2) + " | Dis:" + parseFloat(data.discount).toFixed(2);
                    var billSummary = "";// + " | Dis:" + parseFloat(data.discount).toFixed(2);
                    var printData = {
                        CompanyName: CONTEXT.COMPANY_NAME,
                        Address: CONTEXT.COMPANY_ADDRESS_LINE1.substring(0, 15),
                        ...
                        BillType: bill_title,
                        //BillNo: bill_no,
                        BillNo: data.order_id,
                        Customer: (data.cust_name == null) ? "" : "Customer : " + data.cust_name + "",
                        CustId: (data.cust_no == null) ? "" : data.cust_no,
                        BillDate: "Date : " + data.bill_date,
                        BillTime: currentDate,
                        SubTotal: "Total Items : " + data_item.length + "",
                        Discount: "",
                        Tax: "",
                        TotalQty: "Total Qty : " + tot_qty + "",
                        TotalAmount: "",
                        //BarCodeText: getBitByZero(bill_no, 10),
                        BarCodeText: getBitByZero(data.bill_id, 10),
                        Template: CONTEXT.SO_RECEIPT_TEMPLATE_FORMAT,
                        ProductHeading1: convertBitString("S.No", 6) + " " + convertBitString("Item", 22) + " Qty",
                        ProductHeading2: "",
                        ProductHeading3: "",
                        ProductHeading4: convertBitString(""),
                        ProductHeading5: "",//convertBitString("Rate", 8),
                        ProductHeading6: "",
                        EnableBarcode: "false",
                        FooterText: "",
                    ...
                    if (print_bill) {
                        ...
                        alert("All Bill Items Are Fulfilled..!");
                    }
                    page.salesService.updateSendFulfillItems(order_id, function (temp_data) {
                        page.loadSalesOrder(page.selectedSO.order_id, function () { });
                    });
                });
            });
        ...
        page.printJasper = function (bill_no, exp_type) {
            //page.billAPI.getValue(bill_no, function (data) {
            //    page.events.btnprintInvoiceTemplate2_click(data, exp_type);
            //});
            var billdata = {
                bill_no: bill_no,
            }
            page.billService.getSalesPrint(billdata, function (data) {
                page.events.btnPrintInvoiceTemplate19(data, exp_type);
            });
        }
        page.events.btnprintInvoiceTemplate2_click = function (billItem, exp_type) {
            var data = billItem;
            var bill_item = [];
            //var pending_amt;
            //var advance_pay = billItem.advance_pay;
            //var current_pay = billItem.current_pay;
            var paid = 0;
            if (billItem.payments.length > 0) {
                $(billItem.payments).each(function (i, item) {
                    paid = parseFloat(paid) + parseFloat(item.amount);
                })
            } 
            var balance = parseFloat(billItem.total) - parseFloat(paid);
            var s_no = 0;
            var total_qty = 0,item_rate=0;
            var no_of_items_length = billItem.bill_items.length;
            $(billItem.bill_items).each(function (i, item) {
                s_no = s_no + 1;
                item.price = (isNaN(item.price)) ? 0 : item.price;
                item.total_price = (isNaN(item.total_price)) ? 0 : item.total_price;
                item.item_sub_total = (isNaN(item.item_sub_total)) ? 0 : item.item_sub_total;
                item.discount = (isNaN(item.discount)) ? 0 : item.discount;
                total_qty = parseFloat(item.qty) + parseFloat(total_qty);
                if (item.unit_identity == "1") {
                ...
                if (page.PrintBillType == "Sale") {
                    if (item.tax_inclusive == "1") {
                        item.price = parseFloat(item.price) - parseFloat(item.cess_qty_amount);
                        item.price = parseFloat(parseFloat(parseFloat(item.price) - (parseFloat(item.discount) / item.qty)) / parseFloat((parseFloat(item.tax_per) / 100) + 1)).toFixed(CONTEXT.COUNT_AFTER_POINTS);
                    }
                }
                bill_item.push({
                    "BillItemNo": s_no,
                    ...
                    "Hsn": item.hsn_code,
                    "GST": parseInt(item.tax_per) + "%",//parseFloat(item.item_gst_tax).toFixed(5),
                    ...
                    "GST Amt": (data.state_text == "Return") ? parseFloat(parseFloat(parseFloat(item.total_price) * parseFloat(item.tax_per) / 100)).toFixed(CONTEXT.COUNT_AFTER_POINTS) : parseFloat(parseFloat(item.total_price).toFixed(CONTEXT.COUNT_AFTER_POINTS) - parseFloat(item.item_sub_total).toFixed(CONTEXT.COUNT_AFTER_POINTS) + parseFloat(item.discount).toFixed(CONTEXT.COUNT_AFTER_POINTS)).toFixed(CONTEXT.COUNT_AFTER_POINTS),
            ...
            var accountInfo =
                {
                    ...
                    "NoofItems": no_of_items_length,
                    "Quantity": total_qty,
                    "BSubTotal": parseFloat(data.sub_total).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                    "DiscountAmount": parseFloat(data.discount).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                    ...
                    "TaxAmount": parseFloat(data.tax).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                    "GST Amt": parseFloat(data.tax).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                    "BillAmount": parseFloat(data.total).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                    //"AdvanceAmount": advance_pay,
                    //"CurrentAmount": current_pay,
                    "PaidAmount": parseFloat(paid).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                    "PendingAmount": parseFloat(balance).toFixed(CONTEXT.COUNT_AFTER_POINTS),
            ...
            if (page.PrintBillType == "SaleReturn") {
                ...
                GeneratePrint("ShopOnDev", "sales-bill-print/velan-main-sales-bill.jrxml", accountInfo, exp_type, function (responseData) {
                    if (page.printInvoiceCheck == "button") {
                        $$("pnlPrintingInvoicePopup").close();
                    } else {
        ...
        page.events.btnprintInvoiceTemplate1_click = function (billItem, exp_type) {
            var data = billItem[0];
            var bill_item = [];
            //var pending_amt;
            var advance_pay = billItem[0].advance_pay;
            var current_pay = billItem[0].current_pay;
            var paid = billItem[0].paid;
            var balance = billItem[0].total - billItem[0].paid;
            var s_no = 0;
            $(billItem).each(function (i, item) {
                s_no = s_no + 1;
                //pending_amt = parseFloat(item.total_price) - parseFloat(paid);
                if (item.unit_identity == "1") {
                    item.qty = parseFloat(item.qty) / parseFloat(item.alter_unit_fact);
                    item.free_qty = parseFloat(item.qtyfree_qty) / parseFloat(item.alter_unit_fact);
                }
                bill_item.push({
                    ...
                    "CGST": parseInt(item.tax_per) + "%",//parseFloat(item.item_gst_tax).toFixed(5),
                    "SGST": parseInt(item.tax_per) + "%",//parseFloat(item.item_gst_tax).toFixed(5),
                    "GST Amt": parseFloat(item.total_price),
                    "GValue": parseFloat(item.total_price).toFixed(CONTEXT.COUNT_AFTER_POINTS)
                });
            });
            var accountInfo =
                        {
                            ...
                            "GST Amt": parseFloat(data.tot_tax_amt).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                            "BillAmount": parseFloat(data.total).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                            "AdvanceAmount": advance_pay,
                            "CurrentAmount": current_pay,
                            "PaidAmount": parseFloat(paid).toFixed(CONTEXT.COUNT_AFTER_POINTS),
            ...
            if (page.PrintBillType == "SaleReturn") {
                accountInfo.BillName = "SALES RETURN BILL";
                if (CONTEXT.SALES_ORDER_PRINT == "Template1") {
                    GeneratePrint("ShopOnDev", "sales-bill-print/main-sales-bill.jrxml", accountInfo, exp_type, function (responseData) {
                        $$("pnlPrintingPopup").close();
                        $$("pnlBillViewPopup").open();
                        $$("pnlBillViewPopup").title("Bill View");
                        $$("pnlBillViewPopup").rlabel("Bill View");
        ...
        page.events.btnPrintInvoiceTemplate19 = function (billItem, exp_type) {
            var data = billItem[0];
            var bill_hsn = [];
            var bill_item = [];
            var s_no = 0;
                ...
                if (true) {
                    ...
                    if (item.hsn_code == old_hsn_code) {
                        ...
                        bill_hsn.push({
                            ...
                            "Taxable_Value": parseFloat(item.item_sub_total).toFixed(CONTEXT.COUNT_AFTER_POINTS),//parseFloat(item.alter_unit_fact)) * parseFloat(item.price)).toFixed(2),
                            "CRate": item.cgst_per,
            ...
            var accountInfo =
                {
                    ...
                    "BillDate Name": "Dated",
                    "BillDate": data.bill_date,
                    "Delivery Note Name": "Status",
                    "Delivery Note": $$("lblSOState").html(),
                    "Mode/Terms Of Payment Name": "",
                    "Mode Of Payment": "",
                    "Supplier ref Name": "Order No",
                    "Supplier ref": page.selectedSO.order_id,
                    "Other References Name": "",
                    "Other References": "",
                    "Buyer Order No Name": "",
                    "Buyer Order No": "",
                    "Buyer Dated Name": "",
                    "Buyer Dated": "",
                    "Despatch Document No Name": "",
                    "Despatch Document No": "",
                    "Delivery Note Date Name": "",
                    "Delivery Note Date": "",
                    "Despatched through Name": "",
                    "Despatched through": "",
                    "Destination Name": "",
                    "Destination": "",
            ...
            GeneratePrint("ShopOnDev", "sales-bill-print/main_sales_template19-salesorder.jrxml", accountInfo, exp_type, function (responseData) {
                $$("pnlBillViewPopup").open();
                $$("pnlBillViewPopup").title("Bill View");
                $$("pnlBillViewPopup").rlabel("Bill View");
                $$("pnlBillViewPopup").width("1000");
        ...
        page.events.btnInvoiceSO_click = function () {
            ...
            page.controls.pnlInvoiceSOPopup.height(250);
            (CONTEXT.ENABLE_ORDER_RECEIPT_PRINT) ? $$("pnlReceiptPrint").show() : $$("pnlReceiptPrint").hide();
        }
            ...
            if (CONTEXT.ENABLE_INVOCE_SMS == "true") {
                try {
                    ...
                    var ajaxInfo = {
                        ...
                        "senderNumber": CONTEXT.SMS_SENDER_NO,
                        "companyId": CONTEXT.SMS_COMPANY_ID,
                        "receiverNumber": "+91"+$$("txtContactNo").value(),
                        "message":
        ...
        page.events.btnNewSO_click = function () {
            ...
            $$("ddlCustomerName").selectedObject.val("");
            $$("dsExpectedDate").setDate($.datepicker.formatDate("mm-dd-yy", new Date()));
            page.newCustno = null;
            $$("ddlCustomerName").selectedObject.focus();
        }

        //Show popup to pay invoice
        page.events.btnPayInvoiceAmount_click = function () {
            if (parseFloat($$("lblTotalAmtRemaining").value()) == 0 || (parseFloat($$("lblTotalAmtRemaining").value()) < parseFloat($$("lblReturnsBalance").value()))) {
                page.controls.pnlPaymentAlert.open();
                page.controls.pnlPaymentAlert.title("Payment Alert");
                page.controls.pnlPaymentAlert.rlabel("Payment Alert");
                page.controls.pnlPaymentAlert.width(550);
                page.controls.pnlPaymentAlert.height(170);
            }
            else {
                ...
                $$("dsBillPayDate").setDate($.datepicker.formatDate("mm-dd-yy", new Date()));  // Load Current SO when click Pay
                $$("txtPayDesc").value("CurrentSO");
                //$$("txtPayAmount").value(page.remainingAmount);
                $$("txtPayAmount").value(parseFloat($$("lblTotalAmtRemaining").value()) - parseFloat($$("lblReturnsBalance").value()));
            }
        }
        page.events.btnClosePaymentPopup_click = function () {
            page.controls.pnlPaymentAlert.close();
        }
        ...
        page.events.btnReturnPayInvoiceAmount_click = function () {
            if ($$("lblReturns").html() == 0) {
                page.controls.pnlReturnAlert.open();
                page.controls.pnlReturnAlert.title("Return Alert");
                page.controls.pnlReturnAlert.rlabel("Return Alert");
                page.controls.pnlReturnAlert.width(550);
                page.controls.pnlReturnAlert.height(150);
            }
            else if ($$("lblReturnsBalance").value() == 0) {
                page.controls.pnlSettlementAlert.open();
                page.controls.pnlSettlementAlert.title("Settlement Alert");
                page.controls.pnlSettlementAlert.rlabel("Settlement Alert");
                page.controls.pnlSettlementAlert.width(550);
                page.controls.pnlSettlementAlert.height(150);
            }
            else {
                try {
                    ...
                    page.billService.getReturnedActiveBillByNo(page.selectedSO.order_id, function (data) {
                        $$("ddlReturnBillNo").dataBind(data, "bill_no", "bill_code", "select");
                    });
                    $$("ddlReturnBillNo").selectionChange(function () {
                        ...
                        page.billService.getReturnedBillAmtByNo(data, function (data) {
                            data[0].paid = (data[0].paid == null) ? "0" : data[0].paid;
                            $$("txtReturnBillAmount").value(parseFloat(data[0].total)-parseFloat(data[0].paid));
                            $$("txtReturnPayAmount").value(parseFloat(data[0].total) - parseFloat(data[0].paid));
                            //Amount Refund througth online
                            if (item.bill_type == "SaleReturn") {
                                if (CONTEXT.AUTOMATIC_REFUND) {

                                    if (parseFloat(data[0].total) == parseFloat($$("txtReturnPayAmount").value())) {
                                        var payData = {
                                            companyId: CONTEXT.RAZOR_PAY_COMP_ID,
                                            paymentId: data[0].payment_id
                                        }
                                        $.server.webMethodPOSTDirectURL(CONTEXT.RAZOR_PAY_FULL_REFUND_URL, payData, function (data) {
                                            console.log("Full Refund success...!");
                                        }, function (error) {
                                            console.log(error);
                                        })
                                    } else {
                                        var payData = {
                                            companyId: CONTEXT.RAZOR_PAY_COMP_ID,
                                            paymentId: data[0].payment_id,
                                            amount: parseFloat(parseFloat($$("txtReturnPayAmount").value())*100).toFixed(2)
                                        }
                                        $.server.webMethodPOSTDirectURL(CONTEXT.RAZOR_PAY_PARTIAL_REFUND_URL, payData, function (data) {
                                            console.log("Partial Refund success...!");
                                        }, function (error) {
                                            console.log(error);
                                        })
                                    }
                    ...
                    page.controls.pnlReturnBillPayPopup.height(460);
                    $$("dsReturnBillPayDate").setDate($.datepicker.formatDate("mm-dd-yy", new Date()));  // Load Current SO when click Pay
        ...
        page.events.btnCloseSettlementPopup_click = function () {
            page.controls.pnlSettlementAlert.close();
        }
        page.events.btnReturnAlertPopup_click = function () {
            page.controls.pnlReturnAlert.close();
        }
        ...
        page.events.btnReturnSO_click = function () {
            ...
            page.controls.pnlReturnSOPopup.width(1200);
            page.controls.pnlReturnSOPopup.height(450);
            $$("txtSOIReturnBillDate").setDate($.datepicker.formatDate("mm-dd-yy", new Date()));
            if ($$("ddlDeliveryBy").selectedValue() != "" && $$("ddlDeliveryBy").selectedValue() != "-1")
                $$("ddlReturnedBy").selectedValue($$("ddlDeliveryBy").selectedValue());
            else
                $$("ddlReturnedBy").selectedValue("-1");
            page.billService.getReturnedBillByNo(page.selectedSO.order_id, page.controls.lblSOIBillNo.html(), function (rdata1) {
                var rdata = (rdata1.length == 1) ? rdata1 : getUnique(rdata1);
                $(rdata).each(function (i, item) {
        ...
        page.events.btnReturnSOItemPopup_click = function (overrideItems ,callback) {
            ...
            try {
                ...
                var newBill = {
                    so_no: page.selectedSO.order_id,
                    bill_no:"0",
                    bill_date: ($$("txtSOIReturnBillDate").getDate()=="")?"":dbDateTime($$("txtSOIReturnBillDate").getDate()),
                    store_no: getCookie("user_store_id"),
                    fulfillment_store_no: localStorage.getItem("user_fulfillment_store_no"),
                    storewise_bill: CONTEXT.STOREWISE_BILL_NO,
                ...
                $(itemlList).each(function (i, item) {
                    ...
                    if (item.price == null || item.price == "" || typeof item.price == "undefined" || isNaN(item.price)) {
                        item.price = 0;
                    }
                    if (item.item_price == null || item.item_price == "" || typeof item.item_price == "undefined" || isNaN(item.item_price)) {
                        item.item_price = 0;
                    }
                    ...
                    item.qty_returned = (isNaN(item.qty_returned) || item.qty_returned == null) ? "0" : item.qty_returned;
                    ...
                    if ((parseFloat(item.ret_qty) + parseFloat(item.ret_free)) > 0) {
                        ...
                        if (typeof overrideItems != "undefined") {
                            if (page.cancelso)
                                buying_cost = buying_cost + parseFloat(item.cost) * (parseFloat(item.ordered_qty) + parseFloat(item.free_qty));
                            else
                        ...
                        if (typeof overrideItems != "undefined") {
                            ...
                            lastItem.fin_ret_qty = parseFloat(item.ret_qty) - parseFloat(lastItem.sum_qty_returned);
                            lastItem.fin_ret_free = parseFloat(item.ret_free) - parseFloat(lastItem.free_qty_return);
                            ...
                            newBill.total = parseFloat(newBill.sub_total + newBill.tax);
                        }
                    }
                });
                newBill.sub_total = (isNaN(newBill.sub_total) || newBill.sub_total == null) ? 0 : newBill.sub_total;
                newBill.total = (isNaN(newBill.total) || newBill.total == null) ? 0 : newBill.total;
                newBill.tax = (isNaN(newBill.tax) || newBill.tax == null) ? 0 : newBill.tax;
                var total_after_Rnd_off = Math.round(parseFloat(newBill.total));
                var round_off = parseFloat(parseFloat(total_after_Rnd_off) - parseFloat(newBill.total)).toFixed(CONTEXT.COUNT_AFTER_POINTS);

                newBill.round_off = parseFloat(round_off).toFixed(CONTEXT.COUNT_AFTER_POINTS);
                newBill.total = parseFloat(total_after_Rnd_off).toFixed(CONTEXT.COUNT_AFTER_POINTS);
                if (flagNoData == false)
                    if (!page.cancelso)
                ...
                $(returnItems).each(function (i, billItem) {
                    page.returnItems = page.returnItems + billItem.item_name + ",";
                    //billItem.item_price = (isNaN(billItem.item_price) || billItem.item_price == null) ? 0 : billItem.item_price;
                        rbillItems.push({
                            sl_no: i + 1,
                            ...
                            discount: 0,
                            taxable_value: ((parseFloat(billItem.item_price) * parseFloat(billItem.fin_ret_qty)) * parseFloat(billItem.tax_per)) / 100,
                            sub_total: parseFloat(billItem.item_price) * parseFloat(billItem.fin_ret_qty),
                            tax_per: billItem.tax_per,
                            //total_price: Math.round(parseFloat(parseFloat(billItem.item_price) * parseFloat(billItem.fin_ret_qty) + (((parseFloat(billItem.item_price) * parseFloat(billItem.fin_ret_qty)) * parseFloat(billItem.tax_per)) / 100)).toFixed(2)),//parseFloat(billItem.item_price) * parseFloat(billItem.fin_ret_qty),//billItem.fin_total_price,
                            total_price: parseFloat(parseFloat(billItem.item_price) * parseFloat(billItem.fin_ret_qty) + (((parseFloat(billItem.item_price) * parseFloat(billItem.fin_ret_qty)) * parseFloat(billItem.tax_per)) / 100)).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                            bill_type: "SaleReturn",
                            ...
                            attr_value1: (typeof billItem.attr_value1 == "undefined" || billItem.attr_value1 == null || billItem.attr_value1 == "") ? "" : billItem.attr_value1,
                            attr_type2: (typeof billItem.attr_type2 == "undefined" || billItem.attr_type2 == null || billItem.attr_type2 == "") ? "" : billItem.attr_type2,
                            attr_value2: (typeof billItem.attr_value2 == "undefined" || billItem.attr_value2 == null || billItem.attr_value2 == "") ? "" : billItem.attr_value2,
                            attr_type3: (typeof billItem.attr_type3 == "undefined" || billItem.attr_type3 == null || billItem.attr_type3 == "") ? "" : billItem.attr_type3,
                            attr_value3: (typeof billItem.attr_value3 == "undefined" || billItem.attr_value3 == null || billItem.attr_value3 == "") ? "" : billItem.attr_value3,
                            attr_type4: (typeof billItem.attr_type4 == "undefined" || billItem.attr_type4 == null || billItem.attr_type4 == "") ? "" : billItem.attr_type4,
                            attr_value4: (typeof billItem.attr_value4 == "undefined" || billItem.attr_value4 == null || billItem.attr_value4 == "") ? "" : billItem.attr_value4,
                            attr_type5: (typeof billItem.attr_type5 == "undefined" || billItem.attr_type5 == null || billItem.attr_type5 == "") ? "" : billItem.attr_type5,
                            attr_value5: (typeof billItem.attr_value5 == "undefined" || billItem.attr_value5 == null || billItem.attr_value5 == "") ? "" : billItem.attr_value5,
                            attr_type6: (typeof billItem.attr_type6 == "undefined" || billItem.attr_type6 == null || billItem.attr_type6 == "") ? "" : billItem.attr_type6,
                            attr_value6: (typeof billItem.attr_value6 == "undefined" || billItem.attr_value6 == null || billItem.attr_value6 == "") ? "" : billItem.attr_value6,
                            sku_no: billItem.sku_no,
                            serial_no: billItem.serial_no,
                ...
                if (typeof overrideItems != "undefined") {
                    if (page.cancelso) {
                        newBill.sub_total = page.controls.lblSubTotal.value();
                        newBill.round_off = page.controls.lblRndOff.value();
                        newBill.total = page.controls.lblTotal.value();
                        newBill.discount = page.controls.lblDiscount.value();
                        newBill.tax = page.controls.lblTax.value();
                    }
                    var billSO = [];
                    billSO.push({
                        pay_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                        pay_desc: "Sales Order Bill Payment",
                        amount: parseFloat(newBill.total).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                    ...
                    page.stockAPI.insertBill(newBill, function (data) {
                        ...
                        $(stockItems).each(function (i, billItem) {
                            inventItems.push({
                                ...
                                jrn_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                                invent_type: "SaleReturnCredit",
                                description: "Sales Order Return" + bill_code,
                        ...
                        //page.inventoryService.insertInventoryItems(0, inventItems, function (data1) {
                            if (typeof overrideItems == "undefined") {
                                page.loadSalesOrder(page.selectedSO.order_id, function () {
                                    $$("msgPanel").show("Sales order is successfully returned. A new return bill is generated");
                                });
                            }
                            ...
                            var ori_round_off = parseFloat(Math.round(parseFloat(round_off)) - parseFloat(round_off)).toFixed(CONTEXT.COUNT_AFTER_POINTS);
                            var buying_cost_stock = 0;
                            $(page.controls.grdSOItems.allData()).each(function (i, item) {
                                buying_cost_stock = buying_cost_stock + (parseFloat(item.cost) * (parseFloat(item.quantity) + parseFloat(item.free_qty)));
                            });
                            buying_cost = (itemlList.length == 0) ? buying_cost_stock : buying_cost;
                            if (typeof overrideItems != "undefined" && overrideItems.length == 0) {
                                buying_cost = 0;
                                page.salesService.getSOItems(page.selectedSO.order_id, function (data) {
                                    $(data).each(function (i, item) {
                                        item.free_qty = (item.free_qty == null) ? 0 : item.free_qty;
                                        buying_cost = buying_cost + (parseFloat(item.cost) * (parseFloat(item.quantity) + parseFloat(item.free_qty)));
                                    });
                                    var data1 = {
                                        comp_id: localStorage.getItem("user_finfacts_comp_id"),
                            ...
                            if (typeof overrideItems == "undefined") {
                                ...
                                var ajaxInfo = {
                                    ...
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": recNo,
                                    "message": page.returnItems + " in  Your Order No " + $$("lblSORefNo").html() + " is returned.",
                                };
                                if (CONTEXT.ENABLE_SMS_TO_CUST_AFTER_DELIVERY) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                                ...
                                var ajaxInfo = {
                                    ...
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": CONTEXT.SMS_RECEIVER_NO,
                                    "message": page.returnItems + " in " + CONTEXT.SMS_APPNAME + " Order No " + $$("lblSORefNo").html() + " is returned.",
                                };
                                if (CONTEXT.ENABLE_SMS_TO_ADMIN_AFTER_DELIVERY) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                                        ...
                                        page.salesExecutiveRewardService.getRewardById(data[0].reward_plan_id, function (data2) {
                                            if (data2.length != 0) {
                                                page.billService.getBill(page.currentReturnBillNo, function (data1) {
                                                    newItem.sales_executive_no = sales_executive_no;
                                                    newItem.reward_plan_id = data[0].reward_plan_id;
                                                    newItem.bill_no = page.currentReturnBillNo;
                                                    newItem.trans_date = $$("txtSOIReturnBillDate").getDate();
                                                    newItem.points = data1[0].total / data2[0].reward_plan_point;
                                                    newItem.trans_type = "Debit";
                                                    newItem.description = "Debit";
                                                    newItem.setteled_amount = parseFloat(data1[0].total * data2[0].reward_plan_point) / 4;
                                                    page.salesExecutiveRewardService.insertSalesExecutiveRewardt(newItem, function (data) {
                                                        $$("msgPanel").show("Sales Executive Points reduced successfully.");
                                                        //$$("msgPanel").hide();
                                                    });
                                                });
                                            } else {
                                ...
                                page.customerService.getCustomerById(page.controls.ddlSOCustomer.selectedValue(), function (data) {
                                    page.rewardService.getRewardById(data[0].reward_plan_id, function (data2) {
                                        if (data2.length != 0) {
                                            ...
                                            newItem.bill_no = page.currentReturnBillNo;
                                            newItem.trans_date = $$("txtSOIReturnBillDate").getDate();
                                            newItem.points = newBill.total / data2[0].reward_plan_point;
                                            newItem.trans_type = "Debit";
                                            newItem.setteled_amount = parseFloat(newBill.total * data2[0].reward_plan_point) / 4;
                                            newItem.description = "Bill No:" + bill_code;
                                            page.customerService.insertCustomerRewardt(newItem, function (data) {
                                                $$("msgPanel").flash("Customer Points reduced successfully.");
                                            });
                                        }
                                    });
                                });
                            ...
                            $$("msgPanel").hide();
                            if (typeof overrideItems != "undefined") {
                                if (page.allowReturnPayment) {
                                    var allBillSO = [], data2 = [];
                                    allBillSO.push({
                                        collector_id: localStorage.getItem("user_register_id"),
                                        pay_desc: "Sales Order Bill Payment",
                                        amount: parseFloat($$("lblTotalAmtRemaining").value()),
                                        bill_no: $$("lblSOIBillNo").html(),
                                        pay_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                                        pay_type: "Sale",
                                        pay_mode: "Cash",
                                    ...
                                    data2.push({
                                        per_id: CONTEXT.FINFACTS_CURRENT_PERIOD,
                                        target_acc_id: CONTEXT.FINFACTS_SALES_DEF_CASH_ACCOUNT,
                                        paid_amount: parseFloat($$("lblTotalAmtRemaining").value()),
                                        description: "Sales Order-" + bill_code,
                                        jrn_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                                        key_1: $$("lblSOIBillNo").html(),
                                        key_2: page.selectedSO.cust_no,
                                        comp_id: localStorage.getItem("user_finfacts_comp_id"),
                                    ...
                                    var data = {
                                        receive_amount: parseFloat($$("lblTotalAmtRemaining").value()),
                                        bill_amount: parseFloat($$("lblTotalAmtRemaining").value()),
                                        payment_data: allBillSO,
                                        finfacts_sale_data: data2,
                                        finfacts_return_data: [],
                                        discount_amount: [],
                                        finfacts_expense: [],
                                        bill_no_par: $$("lblSOIBillNo").html()
                                    }
                                    page.billPaymentAPI.postAllValue(data, function (temp_data) {
                                        page.billService.getBillBySO(page.selectedSO.order_id, function (data) {
                                            ...
                                            $(data).each(function (i, item) {
                                                if (item.bill_type == "SaleReturn") {
                                                    if (parseFloat(item.balance) > 0) {
                                                        totRefund = totRefund + parseFloat(item.balance);
                                                        allBillSO.push({
                                                            collector_id: localStorage.getItem("user_register_id"),
                                                            pay_desc: "Sales Order Return Bill Payment",
                                                        ...
                                                        data2.push({
                                                            per_id: CONTEXT.FINFACTS_CURRENT_PERIOD,
                                                            target_acc_id: CONTEXT.FINFACTS_SALES_DEF_CASH_ACCOUNT,
                                                            paid_amount: parseFloat(item.balance),
                                                            description: "Saels Order Return-" + bill_code,
                                                            jrn_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                                                            key_1: page.currentReturnBillNo,
                                                            key_2: page.selectedSO.cust_no,
                                                            comp_id: localStorage.getItem("user_finfacts_comp_id"),
                                            ...
                                            var data = {
                                                receive_amount: parseFloat(newBill.total),
                                                bill_amount: parseFloat(newBill.total),
                                                payment_data: allBillSO,
                                                finfacts_sale_data: [],
                                                finfacts_return_data: data2,
                                                discount_amount: [],
                                            ...
                                            page.billPaymentAPI.postAllValue(data, function (data) {
                                                ...
                                                $$("msgPanel").flash("Sales Order Return payment is recorded successfully.");
                                                var ajaxInfo = {
                                                    "appName": CONTEXT.SMS_APPNAME,
                                                    "senderNumber": CONTEXT.SMS_SENDER_NO,
                                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                                    "receiverNumber": recNo,
                                                    "message": "Amount Rs." + totRefund + " is refunded for Order No " + $$("lblSORefNo").html() + ".",
                                                };
                                                if (CONTEXT.ENABLE_SMS_TO_CUST_AFTER_CANCEL) {
                                                    sendSMS(ajaxInfo, function (response) {
                                                        console.log("Order SMS has been sent..!");
                        ...
                        if (typeof overrideItems != "undefined" && overrideItems.length == 0) {
                            page.salesService.getSOItems(page.selectedSO.order_id, function (data) {
                                ...
                                page.salesItemAPI.postAllValue(0, bill_item, function (data) { });
                            });
                        }
        ...
        page.events.btnSOEvents_click = function () {
            page.controls.pnlSOEvents.open();
            page.controls.pnlSOEvents.title("SO Events");
            page.controls.pnlSOEvents.width(1200);
            page.controls.pnlSOEvents.height(500);
            $$("ddlSOEventStatus").selectedValue("Open");
            $$("ddlSOEventStatus").selectionChange(function () {
                page.view.viewSOEvents();
            })
            page.view.viewSOEvents();
        ...
        page.events.btnPendingPayment_click = function () {
            ...
            page.controls.pnlPayPending.width(1300);
            page.controls.pnlPayPending.height(600);
            ...
            $$("ddlBillView").selectionChange(function () {
                ...
                loan_data.order_id = $$("txtSearchBillNo").value();
                ...
                if ($$("ddlBillView").selectedValue() == "Payment View") {
                    ...
                    filter.order_id = $$("txtSearchBillNo").value();
                    ...
                    page.billService.getPendingReport(filter, function (data) {
                        ...
                        $(data).each(function (i, item) {
                            if (parseFloat(item.total) != parseFloat(item.total_paid_amount)) {
                                if (item.order_no != null && item.order_no != "" && typeof item.order_no != "undefined") {
                                    payment_data.push({
                                        bill_no: item.bill_no,
        ...
        page.events.btnbtnSearchPending_click = function () {
            page.view.selectedPendingPayment([]);
            if ($$("ddlBillView").selectedValue() == "Payment View") {
                var loan_data = {};
                loan_data.cust_no = $$("ddlCustName").selectedValue() == -1 ? "" : $$("ddlCustName").selectedValue();
                ...
                page.billService.getPendingReport(filter, function (data) {
                    ...
                    if (payment_data.length == 0) {
                        alert("Please Check Your Order No");
                    }
                    else if (payment_data.length == 1) {
        ...
        page.events.btnPayPending_click = function () {
            ...
            try {
                ...
                $(page.controls.grdPendingPayment.allData()).each(function (i, item) {
                    if (parseFloat(item.amount_pay) > 0) {
                        ...
                        allBillSO.push({
                            ...
                            pay_type: (item.bill_type == "Sale")?"Sale":"SaleReturn",
                        ...
                        if (item.bill_type == "Sale") {
                            data1.push({
                                ...
                                description: "Sales Order-" + item.bill_code,
                                jrn_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                                key_1: item.bill_no,
                                key_2:"",
                                comp_id: localStorage.getItem("user_finfacts_comp_id"),
                        ...
                        if (item.bill_type == "SaleReturn") {
                            data2.push({
                                ...
                                //amount: item.amount_pay,
                                paid_amount: item.amount_pay,
                                description: "Sales Order Return-" + item.bill_code,
                                jrn_date: dbDateTime($.datepicker.formatDate("dd-mm-yy", new Date())),
                                key_1: item.bill_no,
                                key_2: "",
                                comp_id: localStorage.getItem("user_finfacts_comp_id"),
                ...
                page.billPaymentAPI.postAllValue(data, function (data) {
                    $$("msgPanel").show("Sales Order payment is recorded successfully.");
                    page.controls.pnlPayPending.close();
                    $$("btnPayPending").disable(false);
                    $$("grdSOResult").dataBind({
                        getData: function (start, end, sortExpression, filterExpression, callback) {
                            page.salesService.getSOs("", "", page.view.searchInput(), $$("ddlState").selectedValue(), $$("ddlOrderState").selectedValue(), function (data) {
                                var totalRecord = data.length;
                                page.salesService.getSOs(start, end, page.view.searchInput(), $$("ddlState").selectedValue(), $$("ddlOrderState").selectedValue(), function (data) {
                ...
                //page.salesService.payAllInvoiceBillSalesOrder(0, allBillSO, function (data) {
                    ...
                    //            //ShowDialogBox('Message', 'Payment Data Updated Successfully...!', 'Ok', '', null);
                    //            alert("Payment Data Updated Successfully...!");
                    //        });
                    //    }
                    //    if (data2.length != 0) {
                    //        page.finfactsEntry.allcreditReturnSalesPayment(0, data2, function (response) {
                    //            //ShowDialogBox('Message', 'Settlement Data Updated Successfully...!', 'Ok', '', null);
                    //            alert("Settlement Data Updated Successfully...!");
                    //        });
                    //    }
                    //    page.controls.pnlPayPending.close();
                    //    $$("btnPayPending").disable(false);
                    ...
                    $(page.controls.grdPendingPayment.allData()).each(function (i, item) {
                        if (parseFloat(item.amount_pay) > 0) {
                            if (item.mobile_no != "" && item.mobile_no != null) {
                                var recNo = "";
                                if (item.mobile_no.indexOf(CONTEXT.SMS_COUNTRY_CODE) > -1) {
                                    recNo = $$("txtContactNo").value();
                                } else {
                                    recNo = CONTEXT.SMS_COUNTRY_CODE + "" + item.mobile_no;
                                }
                                var ajaxInfo = {
                                    "appName": CONTEXT.SMS_APPNAME,
                                    "senderNumber": CONTEXT.SMS_SENDER_NO,
                                    "companyId": CONTEXT.SMS_COMPANY_ID,
                                    "receiverNumber": recNo,
                                    "message": "Amount Rs." + item.amount_pay + " is paid for your Order No " + item.order_ref_no + ".",
                                };
                                if (item.bill_type == "SaleReturn") {
                                    ajaxInfo.message = "Amount Rs." + item.amount_pay + " is refunded for Order No " + item.order_ref_no + ".";
                                }
                                if (CONTEXT.ENABLE_SMS_TO_CUST_AFTER_CANCEL) {
                                    sendSMS(ajaxInfo, function (response) {
                                        console.log("Order SMS has been sent..!");
                            ...
                            var ajaxInfo = {
                                ...
                                "companyId": CONTEXT.SMS_COMPANY_ID,
                                "receiverNumber": CONTEXT.SMS_RECEIVER_NO,
                                "message": CONTEXT.SMS_APPNAME + " Order No " + item.order_ref_no + " amount Rs." + item.amount_pay + " is refunded",
                            };
                            if (CONTEXT.ENABLE_SMS_TO_ADMIN_AFTER_CANCEL) {
                                sendSMS(ajaxInfo, function (response) {
                                    console.log("Order SMS has been sent..!");
                            ...
                            if (item.bill_type == "SaleReturn") {
                                if (CONTEXT.AUTOMATIC_REFUND) {
                                    ...
                                    if (parseFloat(item.total) == parseFloat(item.amount_pay)) {
                                        var payData = {
                                            companyId: CONTEXT.RAZOR_PAY_COMP_ID,
                                        ...
                                        var payData = {
                                            companyId: CONTEXT.RAZOR_PAY_COMP_ID,
                                            paymentId: item.payment_id,
                                            amount: parseFloat(parseFloat(item.amount_pay)*100).toFixed(2)
                                        }
                                        $.server.webMethodPOSTDirectURL(CONTEXT.RAZOR_PAY_PARTIAL_REFUND_URL, payData, function (data) {
                                            console.log("Partial Refund success...!");
                                        }, function (error) {
        ...
        page.checkCrossButton = function () {
            //$(page.controls.grdSOItems.getAllRows()).each(function (i, row) {
            //    if (page.removeItemsInGrid) {
            //        $(row).find("[datafield=order_item_id]").find("div").css("display", "block");
            //    }
            //    else {
            //        $(row).find("[datafield=order_item_id]").find("div").css("display", "none");
            //    }
            //});
        }
        page.calculate = function () {
            ...
            $(page.controls.grdSOItems.getAllRows()).each(function (i, row) {
                ...
                curWidth = $(row).find("div[datafield=attributes]").width();
                ...
                var qty,free_qty; // = parseFloat($(row).find("[datafield=quantity]").find("input").val());

                $(alldata).each(function (index, item) {
                    if (i == index) {
                        if (item.qty_type == "Integer"){
                            qty = parseInt(item.quantity);
                            free_qty = parseInt(item.free_qty);
                        }
                        else {
                ...
                disc_price = (isNaN(disc_price) || disc_price == "Infinity") ? "0" : disc_price;
                ...
                discount_val = (isNaN(discount_val) || discount_val == "Infinity") ? "0" : discount_val;
                ...
                discount = (isNaN(discount) || discount == "Infinity") ? "0" : discount;
                
                tot_discount = tot_discount + discount;
                price = parseFloat(parseFloat(price) - (parseFloat(discount) / qty));
                price = (isNaN(price) || price == "Infinity") ? "0" : price;
            ...
            tot_discount = (isNaN(tot_discount)) ? 0 : tot_discount;
            ...
            tot_discount_val = (isNaN(tot_discount_val)) ? 0 : tot_discount_val;
            tot_sales_tax = (isNaN(tot_sales_tax)) ? 0 : tot_sales_tax;
            total = (isNaN(total)) ? 0 : total;

            page.controls.lblDiscount.value(parseFloat(parseFloat(tot_discount_val).toFixed(CONTEXT.COUNT_AFTER_POINTS)));
            ...
            page.controls.lblTotalCopy.value(parseFloat(total_after_Rnd_off).toFixed(CONTEXT.COUNT_AFTER_POINTS));
            ...
            $($$("grdSOItems").selectedObject).find("div[datafield=attributes]").width(maxWidth);
            $$("grdSOItems").selectedObject.find(".grid_bill_footer div[datafield=item_name]").html("Total Qty : " + tot_qty);
        }
        ...
        page.calculate_delivery_amount = function () {
            page.controls.grdDiscount.dataBind(page.discount);
            ...
            $(page.controls.grdSOItems.getAllRows()).each(function (i, row) {
                ...
                $(alldata).each(function (index, item) {
                    if (i == index) {
                        item.delivered = (item.delivered == null) ? 0 : item.delivered;
                        item.free_qty = (item.free_qty == null) ? 0 : item.free_qty;
                        if (parseInt(item.delivered) > 0) {
                            if (item.qty_type == "Integer")
                                qty = parseInt(item.delivered) - parseInt(item.free_qty);
                            else
                                qty = parseFloat(item.delivered) - parseFloat(item.free_qty);
                        }
                        else{
                            qty = 0;
                        }
            ...
            page.controls.lblDeliveryAmount.html(parseFloat(total_after_Rnd_off).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                ...
                if (typeof item.item_no != "undefined") {
                    ...
                    page.itemService.getItemDiscountAutocomplete(item.item_no, function (data) {
                        ...
                        if (item.tax_class_no == null || item.tax_class_no == "" || item.tax_class_no == undefined)
                            item.tax_class_no = -1;
                        $(page.sales_tax).each(function (i, tax_data) {
                        ...
                        var newitem = {
                            ...
                            qty_stock: item.qty_stock,
                            // cost: item.price,
                            cost: (item.tax_inclusive == "0") ? item.price : parseFloat(parseFloat(item.price) / ((100 + parseFloat(item.tax)) / 100)).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                            qty_const: item.qty_stock,
                            mrp: item.mrp,
                            order_id: page.view.selectedSO().order_id,
                            discount: item.discount,
                        ...
                        //        page.controls.grdSOItems.getRowData(rows[0]).temp_qty = parseFloat(txtQty.val());
                        ...
                        //        page.controls.grdSOItems.getRowData(rows[0]).qty = parseFloat(txtQty1.val());
        ...
        page.loadSalesOrder = function (order_id, callback) {
            ...
            page.salesService.getSOByNo(order_id, function (dataitem) {
                ...
                $$("txtDeliveryAddress").value(dataitem[0].delivery_address);
                ...
                page.salesService.getSOItems(dataitem[0].order_id, function (data) {
                    ...
                    if (page.selectedSO.bill_no != null) {
                        ...
                        page.billService.getBill(page.selectedSO.bill_no, function (data) {
                            ...
                            page.controls.lblTotalCopy.value(page.SOBill.total);
                        ...
                        page.billService.getAllSODiscount(order_id, function (data) {
                            ...
                            //ReCalculate Bill Summary
                            page.calculate();
                            page.calculate_delivery_amount();
                        });
                    }
                ...
                page.billService.getBillBySO(page.selectedSO.order_id, function (data) {
                    ...
                    var itemTemp = "<input type='button' class='grid-button' value='' action='Open' style='    border: none;    background-color: transparent;    background-image: url(Open_file.png);    background-size: contain;    width: 25px;    height: 25px;margin-right:10px' /> <input type='button' title='Jasper Print'  class='grid-button' value='' action='PrintJasper' style='    border: none;    background-color: transparent;    background-image: url(print.png);    background-size: contain;    width: 25px;    height: 25px;;margin-right:10px' /> ";

                    if (CONTEXT.ENABLE_SO_RECEIPT_PRINT) {
                        itemTemp = itemTemp + "<input type='button' title='Receipt Print'  class='grid-button' value='' action='PrintReceipt' style='    border: none;    background-color: transparent;    background-image: url(print.png);    background-size: contain;    width: 25px;    height: 25px;;margin-right:10px' />  "
                    }
                    ...
                    $$("grdBillTransactions").setTemplate({
                        ...
                        columns: [
                                    ...
                                    { 'name': "Paid", 'width': "120px", 'dataField': "paid" },
                                    { 'name': "Balance", 'width': "120px", 'dataField': "balance" },
                                    { 'name': "Action", 'width': "120px", 'dataField': "bill_no", editable: false, itemTemplate: itemTemp}
                        ]
                    });
                    page.controls.grdBillTransactions.rowCommand = function (action, actionElement, rowId, row, rowData) {
                        ...
                        if (action == "PrintReceipt") {
                            page.events.btnPrintReceiptTemplate(rowData.bill_no);
                        }
                    }
                    page.controls.grdBillTransactions.dataBind(data);
                });
                ...
                page.events.btnGetReplaceOrders_click();
            });
        }
        ...
        page.events.btnOpenBill_click = function (bill) {
            ...
            page.controls.pnlViewer.width(1000);
        ...
        page.events.page_load = function () {
            ...
            $("[controlid=txtItemSearch]").bind("keypress", function (e) {
                if (e.ctrlKey == true && e.which == 0) {
                    if ($(this).attr("search_mode") == "text") {
                        ...
                        $$("imgBarcode").show();
                        $("[controlid=txtItemSearch]").css("margin-left", "-90px");
                        $$("lblItemSearch").hide();
                    }
                        ...
                        $("[controlid=txtItemSearch]").css("margin-left", "0px");
                        $$("imgBarcode").hide();
                    }
            ...
            page.controls.txtItemSearch.itemTemplate = function (item) {
                ...
                if (CONTEXT.GROUP_BUYING_COST) {
                    if (item.ptype_name == null)
                        return "<a>" + item.item_name + "-" + item.item_code + "<span style='margin:right:30px'><span>[PP:" + parseFloat(item.cost).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "]<span style='margin:right:30px'><span>[SP:" + parseFloat(item.price).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "]<span style='margin:right:30px'>" + attr_text + "</span> </a>";//[MP:" + mrp + "]
                    else
                        return "<a>" + item.item_name + " in " + item.ptype_name + " -" + item.item_code + "<span style='margin:right:30px'><span>[PP:" + parseFloat(item.cost).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "]<span style='margin:right:30px'><span>[SP:" + parseFloat(item.price).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "]<span style='margin:right:30px'>" + attr_text + "</span> </a>";//[MP:" + mrp + "]
                }
                else {
                    if (item.ptype_name == null)
                        return "<a>" + item.item_name + "-" + item.item_code + "<span style='margin:right:30px'><span style='margin:right:30px'><span>[SP:" + parseFloat(item.price).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "]<span style='margin:right:30px'>" + attr_text + "</span> </a>";//[MP:" + mrp + "]
                    else
                        return "<a>" + item.item_name + " in " + item.ptype_name + " -" + item.item_code + "<span style='margin:right:30px'><span style='margin:right:30px'><span>[SP:" + parseFloat(item.price).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "]<span style='margin:right:30px'>" + attr_text + "</span> </a>";//[MP:" + mrp + "]
                }
            }
            ...
            page.controls.txtItemSearch.select(function (item) {
                ...
                page.removeItemsInGrid = true;
                page.checkCrossButton();
            });
            ...
            page.controls.ddlCustomerName.dataBind({
                getData: function (term, callback) {
                    callback(page.customerList);
                }
            });
            page.controls.ddlCustomerName.select(function (item) {
                page.newCustno = item.cust_no;
            });
            ...
            $$("ddlPayFor").selectionChange(function (data) {
                ...
                if ($$("ddlPayFor").selectedValue() == "Current SalesOrder") {
                    ...
                    $$("txtPayAmount").value(parseFloat($$("lblPendingPayment").value()) - (parseFloat($$("lblTotalReturns").value()) - parseFloat($$("lblTotalReturnsPayment").value())));
            ...
            $$("ddlSOCustomer").selectionChange(function () {
                page.customerService.getCustomerById($$("ddlSOCustomer").selectedValue(), function (data) {
                    if ($$("ddlSOCustomer").selectedValue() != page.selectedSO.cust_no) {
                        data[0].company = (data[0].company == null || data[0].company == "") ? "" : data[0].company;
                        data[0].phone_no = (data[0].phone_no == null || data[0].phone_no == "") ? "" : data[0].phone_no;
                        data[0].email = (data[0].email == null || data[0].email == "") ? "" : data[0].email;
                        data[0].gst_no = (data[0].gst_no == null || data[0].gst_no == "") ? "" : data[0].gst_no;
                        data[0].address2 = (data[0].address2 == null || data[0].address2 == "") ? "" : data[0].address2;
                        data[0].address1 = (data[0].address1 == null || data[0].address1 == "") ? "" : data[0].address1;
                        data[0].city = (data[0].city == null || data[0].city == "") ? "" : data[0].city;
                        data[0].zip_code = (data[0].zip_code == null || data[0].zip_code == "") ? "" : data[0].zip_code;
                        ...
                        $$("lblCustomerReward").html(data[0].points);
                        $$("txtGst").value(data[0].gst_no);

                        $$("txtShippingAddress").value(data[0].address2 + "-" + data[0].address1 + "-" + data[0].city + "-" + data[0].zip_code);
                        $$("txtDeliveryAddress").value(data[0].address2 + "-" + data[0].address1 + "-" + data[0].city + "-" + data[0].zip_code);
                    }
                    else {
                        $$("txtSOCompany").value(page.selectedSO.company);
                        $$("txtContactNo").value(page.selectedSO.contact_no);
                        $$("txtEmail").value(page.selectedSO.email);
                        $$("lblCustomerReward").html(page.selectedSO.points);
                        $$("txtGst").value(data[0].gst_no);

                        $$("txtShippingAddress").value(page.selectedSO.delivery_address);
                        $$("txtDeliveryAddress").value(page.selectedSO.delivery_address);
                    }
                });
            ...
            $$("grdSOResult").dataBind({
                getData: function (start, end, sortExpression, filterExpression, callback) {
                    page.salesService.getSOs("", "", page.view.searchInput(), -1, $$("ddlOrderState").selectedValue(), function (data) {
                        var totalRecord = data.length;
                        page.salesService.getSOs(start, end, page.view.searchInput(), -1, $$("ddlOrderState").selectedValue(), function (data) {
                            callback(data, totalRecord);
                        });
                    });
                },
            ...
            $$("ddlOrderState").selectionChange(function () {
                page.salesService.getSOs("", "", page.view.searchInput(), $$("ddlState").selectedValue(), $$("ddlOrderState").selectedValue(), function (data) {
                    page.view.searchResult(data);
                });
            });
            ...
            page.itemAttributeAPI.searchValue(0, "", "attr_no_key in (1,2,3,4,5,6)", "", "", function (data) {
                page.attr_list = data;
            });
            ...
            searchViewData = [];
            searchViewData.push({ view_no: "-1", view_name: "All" }, { view_no: "1", view_name: "Today's Order" }, { view_no: "2", view_name: "Weekly Order" }, { view_no: "3", view_name: "Monthly Order" });
            $$("ddlOrderState").dataBind(searchViewData, "view_no", "view_name");
            $$("ddlOrderState").selectedValue("1");

            $$("ddlExportType").dataBind(CONTEXT.JASPER_SUPPORTING_FORMATS, "value", "value");
            $$("ddlInvoiceExportType").dataBind(CONTEXT.JASPER_SUPPORTING_FORMATS, "value", "value");

            $$("chkShowMore").change(function () {
                if ($$("chkShowMore").prop("checked")) {
                    CONTEXT.ShowMore = true;
                    page.view.selectedSOItems(page.controls.grdSOItems.allData());
                    page.controls.grdSOItems.edit(true);
                }
                else {
                    CONTEXT.ShowMore = false;
            ...
            (CONTEXT.ENABLE_REWARD_MODULE == true) ? $$("pnlCustomerReward").show() : $$("pnlCustomerReward").hide();
        },
        ...
        function PrintingOD(data) {
            ...
            doc.write("<tr><td colspan='9' style=''><div> No.of.Items :  " + no_of_prd + "</div></td><td colspan='4'> <div ><b>Sub Total: </b><span style='float:right'>" + parseFloat(page.controls.lblSubTotal.value()).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "</span> </div></td> </tr>");
            doc.write("<tr><td colspan='9' style=''><div> Quantity : " + tot_Qty + "</div></td><td colspan='4'> <div ><b>Discount Amount: </b><span style='float:right'>" + parseFloat(page.controls.lblDiscount.value()).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "</span> </div></td> </tr>");
            ...
            doc.write("<tr><td colspan='9' style='border:0px'></td><td colspan='4'> <div ><b>Tax Amount: </b><span style='float:right'>" + parseFloat(page.controls.lblTax.value()).toFixed(CONTEXT.COUNT_AFTER_POINTS) + " </span></div></td> </tr>");
            doc.write("<tr><td colspan='9' style='border:0px'></td><td colspan='4'> <div ><b>Round off: </b><span style='float:right'>" + parseFloat(page.controls.lblRndOff.value()).toFixed(CONTEXT.COUNT_AFTER_POINTS) + " </span></div></td> </tr>");
            doc.write("<tr><td colspan='9' style='border:0px'></td><td colspan='4'> <div ><b>BILL AMOUNT: </b><span style='float:right'>" + parseFloat(page.controls.lblTotal.value()).toFixed(CONTEXT.COUNT_AFTER_POINTS) + "</span> </div> </td></tr>");
            doc.write("</tr>");
            doc.write("</table>");
        ...
        page.view = {
            ...
            searchResult: function (data) {
                page.controls.grdSOResult.width("115%");
                //if (page.viewMode == "Desktop")
                page.controls.grdSOResult.height("67vh");
                page.controls.grdSOResult.setTemplate({
                    ...
                    columns: [
                        ...
                        { 'name': "Order No", 'width': "80px", 'dataField': "order_no" },
                        { 'name': "Customer", 'width': "150px", 'dataField': "cust_name" },
                        { 'name': "State", 'width': "100px", 'dataField': "state_text" },
                        { 'name': "Ordered Date", 'width': "110px", 'dataField': "ordered_date" },
                        { 'name': "Expected Date", 'width': "110px", 'dataField': "expected_date" },
                        { 'name': "Delivered Date", 'width': "115px", 'dataField': "delivered_date" },
                        { 'name': "Total Amount", 'width': "105px", 'dataField': "total_amount",visible:($$("ddlState").selectedValue() != "911") },
                        { 'name': "Paid", 'width': "100px", 'dataField': "total_paid_amount", visible: ($$("ddlState").selectedValue() != "911") },
                        { 'name': "Balance", 'width': "100px", 'dataField': "balance", visible: ($$("ddlState").selectedValue() != "911") },
                        { 'name': "Settlement Amount", 'width': "130px", 'dataField': "total_return_amount", visible: ($$("ddlState").selectedValue() != "910") },
                        { 'name': "Settlement Paid", 'width': "120px", 'dataField': "total_return_paid_amount", visible: ($$("ddlState").selectedValue() != "910") },
                        { 'name': "Settlement Balance", 'width': "120px", 'dataField': "return_balance", visible: ($$("ddlState").selectedValue() != "910") },
                       /// { 'name': "Ordered", 'width': "70px", 'dataField': "ordered_date" },
                        { 'name': "", 'width': "0px", 'dataField': "expected_date",visible:false },
                        { 'name': "", 'width': "0px", 'dataField': "delivered_date", visible: false },
                        { 'name': "Ord Ref No", 'width': "200px", "title": "order_ref_no", 'dataField': "order_ref_no", visible: true },
                        { 'name': "Action", 'width': "100px", 'dataField': "order_id", editable: false,visible:false, itemTemplate: "<input type='button' title='Open Bill'  class='grid-button' value='' action='Open' style='    border: none;    background-color: transparent;    background-image: url(Open_file.png);    background-size: contain;    width: 25px;    height: 25px;margin-right:10px' />  <input type='button' title='Receipt Print'  class='grid-button' value='' action='PrintReceipt' style='    border: none;    background-color: transparent;    background-image: url(print.png);    background-size: contain;    width: 25px;    height: 25px;margin-right:10px;' id='receiptPrint' />" }
                    ]
                });
                page.controls.grdSOResult.rowCommand = function (action, actionElement, rowId, row, rowData) {
                    if (action == "Open") {
                        page.events.grdSOResult_select(rowData);
                    }
                    if (action == "PrintReceipt") {
                        try{
                            if (rowData.bill_no == "" || rowData.bill_no == null || typeof rowData.bill_no == "undefined") {
                                throw "Bill Is Not Generated Yet...!";
                            }
                            page.events.btnPrintReceiptTemplate1(rowData.order_id);
                        }
                        catch (e) {
                            alert(e);
                        }
                ...
                page.controls.grdSOResult.rowBound = function (row, item) {
                    if (item.state_text == "Created" || item.state_text == "Ordered" || item.state_text == "Confirmed") {
                        row[0].style.color = "#368605";
                        $(row[0]).css("font-weight", "bold")
                    }
                    else if (item.state_text == "Cancel" || item.state_text == "Removed") {
                        row[0].style.color = "red";
                        $(row[0]).css("font-weight", "bold")
                    }
                    else {
                        if (parseFloat(Math.round(item.total_return_amount)) == parseFloat(item.total_amount)) {
                            row[0].style.color = "red";
                            $(row[0]).css("font-weight", "bold")
                        }
                        else if (parseFloat(item.total_return_amount) > 0) {
                    ...
                    if (parseInt(item.replacement_order) > 0) {
                        row[0].style.color = "#ff00c8";
                        $(row[0]).css("font-weight", "bold");
                    }
                    if (parseInt(item.replaced_order) > 0) {
                        row[0].style.color = "#0014ff";
                        $(row[0]).css("font-weight", "bold");
                    }
                }
            ...
            salesHistory: function () {
                ...
                page.dynaReportService.getSalesReport(repInput, function (data) {
                    ...
                    page.controls.lblTotalSales.html(parseFloat(salSummary.tot_sale).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    page.controls.lblTotalPayment.html(parseFloat(salSummary.tot_pay).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    page.controls.lblTotalReturns.html(parseFloat(salSummary.tot_ret).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    page.controls.lblTotalReturnsPayment.html(parseFloat(salSummary.tot_ret_pay).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    page.controls.lblPendingPayment.html(parseFloat(parseFloat(page.controls.lblTotalSales.html()) - parseFloat(page.controls.lblTotalPayment.html())).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    $$("lblTotalReturnsBalance").html(parseFloat(parseFloat(page.controls.lblTotalReturns.html()) - parseFloat(page.controls.lblTotalReturnsPayment.html())).toFixed(CONTEXT.COUNT_AFTER_POINTS));

                    if (poSummary.tot_sale != undefined)
                        page.controls.lblTotal.value(parseFloat(poSummary.tot_sale).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    if (poSummary.tot_pay != undefined)
                        page.controls.lblTotalAmtPaid.value(parseFloat(poSummary.tot_pay).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    if (poSummary.tot_bal != undefined)
                        page.controls.lblTotalAmtRemaining.value(parseFloat(poSummary.tot_bal).toFixed(CONTEXT.COUNT_AFTER_POINTS));

                    page.controls.lblReturns.html(parseFloat(poSummary.tot_ret).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    page.controls.lblReturnsPaid.value(parseFloat(poSummary.tot_ret_pay).toFixed(CONTEXT.COUNT_AFTER_POINTS));
                    page.controls.lblReturnsBalance.value(parseFloat(poSummary.tot_ret_bal).toFixed(CONTEXT.COUNT_AFTER_POINTS));

                    if (page.selectedSO.state_text == "Delivered" || page.selectedSO.state_text == "Invoice Paid") {
                        if (parseFloat(page.controls.lblReturns.html()) > 0) {
                            $$("btnCompleteReturnSO").hide();
                            page.controls.btnCompleteReturnSO.selectedObject.prev().hide();
                            $$("pnlReturnsSummary").show();
                        } else {
                            $$("btnCompleteReturnSO").show();
                            page.controls.btnCompleteReturnSO.selectedObject.prev().show();
                            $$("pnlReturnsSummary").hide();
                        }
                        if (parseFloat(page.controls.lblReturns.html()) > 0) {
                            $("[controlid=btnReturnPayInvoiceAmount]").addClass("primaryReadButton");
                            $("[controlid=btnReturnPayInvoiceAmount]").removeClass("secondaryButton");
                            $$("btnCancelSO").selectedObject.next().hide();
                            $("[controlid=btnReturnPayInvoiceAmount]").css("margin-right", "10px");
                        } else {
                            $("[controlid=btnReturnPayInvoiceAmount]").removeClass("primaryReadButton");
                            $("[controlid=btnReturnPayInvoiceAmount]").addClass("secondaryButton");
                            $$("btnCancelSO").selectedObject.next().show();
                            $("[controlid=btnReturnPayInvoiceAmount]").css("margin-right", "0px");
                        }
                    }
                    page.calculate_delivery_amount();
            ...
            selectedSO: function (soDetail) {
                if (soDetail) {
                    ...
                    $$("lblSORefNo").html(soDetail.order_ref_no);

                    if (CONTEXT.ENABLE_ESHOPON) {
                        $$("pnlSORefNo").show();
                    } else {
                        $$("pnlSORefNo").hide();
                    }
                    if (soDetail.bill_date != null && soDetail.bill_date != "" && typeof soDetail.bill_date != "undefined")
                        $$("txtSOIBillDate").setDate(dbDate(soDetail.bill_date));
                    ...
                    $$("lblTempSOIBillNo").html(soDetail.bill_code);
                    ...
                    $$("lblCustomerReward").html(soDetail.points);
                    ...
                    //if (soDetail.confirm_date == null || soDetail.confirm_date == "")
                    //    $$("lblSOConfirmedDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date()));
                    ...
                    (soDetail.ordered_date == "" || soDetail.ordered_date == null) ? $$("lblSOOrderedDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date())) : $$("lblSOOrderedDate").setDate(dbDate(soDetail.ordered_date));
                    (soDetail.expected_date == "" || soDetail.expected_date == null) ? $$("dsSOExpectedDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date())) : $$("dsSOExpectedDate").setDate(dbDate(soDetail.ordered_date));
                    (soDetail.confirm_date == "" || soDetail.confirm_date == null) ? $$("lblSOConfirmedDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date())) : $$("lblSOConfirmedDate").setDate(soDetail.confirm_date);
                    (soDetail.delivered_date == "" || soDetail.delivered_date == null) ? $$("lblSODeliveredDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date())) : $$("lblSODeliveredDate").setDate(soDetail.delivered_date);
                    (soDetail.fulfilled_date == "" || soDetail.fulfilled_date == null) ? $$("lblSOFulfilledDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date())) : $$("lblSOFulfilledDate").setDate(soDetail.fulfilled_date);
                    (soDetail.invoice_paid_date == "" || soDetail.invoice_paid_date == null) ? $$("lblSOInvoiceDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date())) : $$("lblSOInvoiceDate").setDate(soDetail.invoice_paid_date);
                    (soDetail.completed_date == "" || soDetail.completed_date == null) ? $$("lblSOCompletedDate").html("") : $$("lblSOCompletedDate").html(soDetail.completed_date);
                    (soDetail.returned_date == "" || soDetail.returned_date == null) ? $$("lblSOReturnedDate").html("") : $$("lblSOReturnedDate").html(soDetail.returned_date);
            ...
            selectedSOItems: function (data) {
                var curWidth = 0;
                var maxWidth = 100;
                var tot_qty = 0, calc_qty = 0, calc_free_qty = 0;
                ...
                if (true) {
                    if (page.fullfillMode == true) {
                        ...
                        page.controls.grdSOItems.width(grid_width+"%");
                        page.controls.grdSOItems.setTemplate({
                            ...
                            columns: [
                                ...
                                { 'name': "Item No", 'rlabel': 'Item No', 'width': "100px", 'dataField': "item_code", editable: false },
                                { 'name': "VAR", 'width': "100px", 'dataField': "var_no" },
                                { 'name': "Item Name", 'width': "200px;word-break: break-word;white-space: pre-wrap;", 'dataField': "item_name", editable: false, itemTemplate: "<div  id='prdItemName' style='height:auto;'></div>" },
                                ...
                                { 'name': "Pending Fulfill", 'width': "120px", 'dataField': "pendingfullfill", editable: false, visible: page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered" },
                                { 'name': "Rack No", 'width': "100px", 'dataField': "rack_no", visible: CONTEXT.ENABLE_RACK },
                                { 'name': "Serial No", 'rlabel': 'Serial No', 'width': "150px", 'dataField': "serial_no", editable: false, visible: CONTEXT.ENABLE_SERIAL_NO_IN_POS },

                                { 'name': "Returned", 'width': "80px", 'dataField': "qty_returned", editable: false, visible: false },
                                { 'name': "Delivered", 'width': "80px", 'dataField': "delivered", visible: false },
                                { 'name': "Tray", 'width': "40px", 'dataField': "tray_count", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "quantity", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "free_qty", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "", 'width': "1px", 'dataField': "order_item_id", editable: false, visible: false },
                                { 'name': "Item No", 'rlabel': 'Item No', 'width': "100px", 'dataField': "item_no", visible: false },
                                { 'name': "Attributes", 'rlabel': 'Attributes', 'dataField': "attributes", 'width': "100px", itemTemplate: "<div  id='Attributes'></div>", visible: false },
                                { 'name': "To Delivered", 'width': "100px", 'dataField': "todelivered", visible: false },
                                { 'name': "Price", 'width': "45px", 'dataField': "price", editable: false, visible: false },
                                { 'name': "Disc", 'width': "50px", 'dataField': "discount", visible: false },
                                { 'name': "GST", 'width': "25px", 'dataField': "tax_per", visible: false },
                                { 'name': "Amount", 'width': "80px", 'dataField': "total_price", visible: false },
                                { 'name': "HSN Code", 'width': "100px", 'dataField': "hsn_code", editable: false, visible: false },
                                { 'name': "CGST", 'width': "80px", 'dataField': "cgst", visible: false },
                                { 'name': "SGST", 'width': "80px", 'dataField': "sgst", visible: false },
                                { 'name': "IGST", 'width': "80px", 'dataField': "igst", visible: false },
                                ...
                                { 'name': "", 'width': "0px", 'dataField': "alter_unit", visible: false },
                                ...
                                { 'name': "", 'width': "0px", 'dataField': "alter_price_1", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "alter_price_2", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "temp_price", visible: false },
                                ...
                                { 'name': "", 'width': "0px", 'dataField': "attr_value1", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_type2", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_value2", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_type3", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_value3", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_type4", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_value4", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_type5", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_value5", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_type6", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_value6", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "send_to_fulfill", visible: false },
                            ]
                        });
                    }
                    else if (page.deliveryMode == true) {
                        page.controls.grdSOItems.width("100%");
                        page.controls.grdSOItems.setTemplate({
                            selection: "Single",
                            columns: [
                                { 'name': "", 'width': "50px", 'dataField': "order_item_id", editable: false, visible: page.selectedSO.state_text == "Created", itemTemplate: "<input type='button' class='grid-button' value='' action='Delete' style='    border: none;    background-color: transparent;    background-image: url(cancel.png);    background-size: contain;    width: 25px;    height: 25px;margin-right:10px' />" },
                                ...
                                { 'name': "Pending Delivery", 'width': "120px", 'dataField': "pendingdelivery", visible: page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered" },


                                { 'name': "Fulfilled", 'width': "80px", 'dataField': "fullfilled", editable: false, visible: false },
                                { 'name': "To Fulfill", 'width': "80px", 'dataField': "tofullfilled", editable: page.selectedSO.state_text == "Confirmed", visible: false },
                                { 'name': "Returned", 'width': "80px", 'dataField': "qty_returned", editable: false, visible: false },
                                { 'name': "In Stock", 'width': "70px", 'dataField': "qty_stock", visible: CONTEXT.SHOW_STOCK_COLUMN, visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "quantity", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "free_qty", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "Serial No", 'rlabel': 'Serial No', 'width': "150px", 'dataField': "serial_no", editable: false, visible: false},
                                { 'name': "Tray", 'width': "40px", 'dataField': "tray_count", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "Price", 'width': "45px", 'dataField': "price", editable: false, visible: false },
                                { 'name': "Disc", 'width': "50px", 'dataField': "discount", visible: false },
                                { 'name': "GST", 'width': "25px", 'dataField': "tax_per", visible: false },
                                ...
                                { 'name': "SGST", 'width': "80px", 'dataField': "sgst", visible: false },
                                { 'name': "IGST", 'width': "80px", 'dataField': "igst", visible: false },
                                { 'name': "Rack No", 'width': "100px", 'dataField': "rack_no", visible: CONTEXT.ENABLE_RACK, visible: false },
                                { 'name': "Attributes", 'rlabel': 'Attributes', 'dataField': "attributes", 'width': "100px", itemTemplate: "<div  id='Attributes'></div>", visible: false },
                                { 'name': "", 'width': "1px", 'dataField': "order_item_id", editable: false, visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "cost", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "tray_id", visible: false },
                        ...
                        page.controls.grdSOItems.setTemplate({
                            ...
                            columns: [
                                ...
                                { 'name': "Item Name", 'width': "200px;word-break: break-word;white-space: pre-wrap;", 'dataField': "item_name", editable: false, itemTemplate: "<div  id='prdItemName' style='height:auto;'></div>" },
                                { 'name': "Qty", 'width': "50px", 'dataField': "temp_qty", editable: page.selectedSO.state_text == "Created" },
                                { 'name': "Free Qty", 'width': "80px", 'dataField': "temp_free_qty", editable: page.selectedSO.state_text == "Created" },
                                { 'name': "Unit", 'width': "75px", 'dataField': "unit", itemTemplate: "<div  id='prdDetail' style=''></div>" },
                                { 'name': "Tray", 'width': "80px", 'dataField': "tray_count", editable: false, visible: CONTEXT.ENABLE_MODULE_TRAY && page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered" },
                                { 'name': "To Tray", 'width': "120px", 'dataField': "tray_received", editable: true, visible: CONTEXT.ENABLE_MODULE_TRAY && page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered" },


                                { 'name': "", 'width': "0px", 'dataField': "quantity", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "free_qty", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "In Stock", 'width': "70px", 'dataField': "qty_stock", visible: false },
                                { 'name': "Serial No", 'rlabel': 'Serial No', 'width': "150px", 'dataField': "serial_no", editable: false, visible: false },
                                { 'name': "Returned", 'width': "80px", 'dataField': "qty_returned", editable: false, visible: false },
                                { 'name': "Fulfilled", 'width': "80px", 'dataField': "fullfilled", editable: false, visible: false },
                                { 'name': "To Fulfill", 'width': "80px", 'dataField': "tofullfilled", editable: page.selectedSO.state_text == "Confirmed", visible: false },
                                { 'name': "Delivered", 'width': "80px", 'dataField': "delivered", editable: false, visible: false },
                                { 'name': "Price", 'width': "45px", 'dataField': "price", editable: false, visible: false },
                                { 'name': "Disc", 'width': "50px", 'dataField': "discount", visible: false },
                                { 'name': "GST", 'width': "25px", 'dataField': "tax_per", visible: false },
                                { 'name': "Amount", 'width': "80px", 'dataField': "total_price", visible: false },
                                { 'name': "Variation", 'width': "120px", 'dataField': "variation_name", editable: false, visible: false },
                                { 'name': "MRP", 'width': "45px", 'dataField': "mrp", editable: false, visible: false },
                                { 'name': "Batch No", 'width': "120px", 'dataField': "batch_no", editable: false, visible: false },
                                { 'name': "Manufacture Date", 'width': "150px", 'dataField': "man_date", editable: false, visible: false },
                                { 'name': "Expiry Date", 'width': "120px", 'dataField': "expiry_date", editable: false, visible: false },
                                { 'name': "HSN Code", 'width': "100px", 'dataField': "hsn_code", editable: false, visible: false },
                                { 'name': "CGST", 'width': "80px", 'dataField': "cgst", visible: false },
                                { 'name': "SGST", 'width': "80px", 'dataField': "sgst", visible: false },
                                { 'name': "IGST", 'width': "80px", 'dataField': "igst", visible: false },
                        ...
                        if (page.selectedSO.state_text == "Created" || page.selectedSO.state_text == "Ordered") {
                            ...
                            if (CONTEXT.ShowMore)
                                page.controls.grdSOItems.width("112%");
                            else
                                page.controls.grdSOItems.width("100%");
                        }
                        page.controls.grdSOItems.setTemplate({
                            selection: "Single",
                            columns: [
                                { 'name': "", 'width': "50px", 'dataField': "order_item_id", editable: false, itemTemplate: "<input type='button' class='grid-button' value='' action='Delete' style='    border: none;    background-color: transparent;    background-image: url(cancel.png);    background-size: contain;    width: 25px;    height: 25px;margin-right:10px' />" },
                                { 'name': "Sl No", 'width': "40px", 'dataField': "sl_no" },
                                { 'name': "SKU", 'rlabel': 'SKU', 'width': "60px", 'dataField': "sku_no", visible: CONTEXT.SHOW_SKU_COLUMN_IN_SALES },
                                { 'name': "Item No", 'rlabel': 'Item No', 'width': "80px", 'dataField': "item_code", editable: false },
                                { 'name': "Item Name", 'width': "200px;word-break: break-word;white-space: pre-wrap;", 'dataField': "item_name", editable: false, itemTemplate: "<div  id='prdItemName' style='height:auto;'></div>" },
                                { 'name': "Qty", 'width': "50px", 'dataField': "temp_qty", editable: true },//page.selectedSO.state_text == "Created"
                                { 'name': "Free Qty", 'width': "70px", 'dataField': "temp_free_qty", editable: true, visible: CONTEXT.SHOW_FREE },//page.selectedSO.state_text == "Created"
                                { 'name': "Unit", 'width': "75px", 'dataField': "unit", itemTemplate: "<div  id='prdDetail' style=''></div>" },// editable: false },
                                { 'name': "Returned", 'width': "65px", 'dataField': "qty_returned", editable: false, visible: page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered" },
                                { 'name': "Fulfilled", 'width': "65px", 'dataField': "fullfilled", editable: false, visible: page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered" },
                                { 'name': "Delivered", 'width': "65px", 'dataField': "delivered", editable: false, visible: page.selectedSO.state_text != "Created" && page.selectedSO.state_text != "Ordered" },
                                ...
                                { 'name': "Disc", 'width': "50px", 'dataField': "discount",visible:CONTEXT.ShowMore },
                                { 'name': "GST", 'width': "50px", 'dataField': "tax_per", visible: CONTEXT.ENABLE_CUST_GST },
                                { 'name': "Amount", 'width': "90px", 'dataField': "total_price" },
                                { 'name': "Serial No", 'rlabel': 'Serial No', 'width': "150px", 'dataField': "serial_no", editable: page.selectedSO.state_text == "Created", visible: CONTEXT.ShowMore && CONTEXT.ENABLE_SERIAL_NO_IN_POS },
                                { 'name': "HSN Code", 'width': "75px", 'dataField': "hsn_code", editable: false, visible: CONTEXT.ShowMore && CONTEXT.ENABLE_CUST_GST },
                                { 'name': "CGST", 'width': "50px", 'dataField': "cgst", visible: CONTEXT.ShowMore && CONTEXT.ENABLE_CUST_GST },
                                { 'name': "SGST", 'width': "50px", 'dataField': "sgst", visible: CONTEXT.ShowMore && CONTEXT.ENABLE_CUST_GST },
                                { 'name': "IGST", 'width': "50px", 'dataField': "igst", visible: CONTEXT.ShowMore && CONTEXT.ENABLE_CUST_GST },
                                ...
                                { 'name': "", 'width': "0px", 'dataField': "free_qty", editable: page.selectedSO.state_text == "Created", visible: false },
                                { 'name': "Attributes", 'rlabel': 'Attributes', 'dataField': "attributes", 'width': "100px", itemTemplate: "<div  id='Attributes'></div>", visible: false },
                                { 'name': "VAR", 'width': "100px", 'dataField': "var_no", visible: false },
                                ...
                                { 'name': "", 'width': "0px", 'dataField': "attr_type6", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "attr_value6", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "new_item", visible: false },
                                { 'name': "", 'width': "0px", 'dataField': "send_to_fulfill", visible: false },
                            ]
                        });
                    }
                    ...
                    page.controls.grdSOItems.setTemplate({
                        selection: "Single",
                        columns: [
                            { 'name': "", 'width': "50px", 'dataField': "order_item_id", editable: false, itemTemplate: "<input type='button' class='grid-button' value='' action='Delete' style='    border: none;    background-color: transparent;    background-image: url(cancel.png);    background-size: contain;    width: 25px;    height: 25px;margin-right:10px' />" },//visible: page.selectedSO.state_text == "Created", 
                            { 'name': "", 'width': "1px", 'dataField': "order_item_id", editable: false, visible: false },
                            { 'name': "Sl No", 'width': "70px", 'dataField': "sl_no" },
                            { 'name': "Item Name", 'width': "300px;word-break: break-word;white-space: pre-wrap;", 'dataField': "item_name", editable: false, itemTemplate: "<div  id='prdItemName' style='height:auto;'></div>" },
                ...
                page.controls.grdSOItems.rowBound = function (row, item) {
                    ...
                    var itemTemplate = [];
                    itemTemplate.push("<h5 class='col-xs-12' style='padding:0px;margin: 0px;'>" + item.item_name + "</h5>");
                    if (item.var_stock_attr_key != null) {
                        if (item.var_stock_attr_key.split(",").length - 1 >= 0) {
                            itemTemplate.push("<br><span style=''>" + item.attr_type1 + ":" + item.attr_value1 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                        }
                        if (item.var_stock_attr_key.split(",").length - 1 >= 1) {
                            itemTemplate.push("<span style=''>" + item.attr_type2 + ":" + item.attr_value2 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                        }
                        if (item.var_stock_attr_key.split(",").length - 1 >= 2) {
                            itemTemplate.push("<span style=''>" + item.attr_type3 + ":" + item.attr_value3 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                        }
                        if (item.var_stock_attr_key.split(",").length - 1 >= 3) {
                            itemTemplate.push("<span style=''>" + item.attr_type4 + ":" + item.attr_value4 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                        }
                        if (item.var_stock_attr_key.split(",").length - 1 >= 4) {
                            itemTemplate.push("<span style=''>" + item.attr_type5 + ":" + item.attr_value5 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                        }
                        if (item.var_stock_attr_key.split(",").length - 1 >= 5) {
                            itemTemplate.push("<span style=''>" + item.attr_type6 + ":" + item.attr_value6 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                        }
                    }
                    ...
                    if (item.new_item == "1") {
                        ...
                        $(row).find("[datafield=order_item_id]").find("div").css("visibility", "hidden");
                    }
                    if ((parseFloat(item.quantity) + parseFloat(item.free_qty)) != parseFloat(item.fullfilled)) {
                        $(row).find("[datafield=fullfilled]").find("div").css("color", "red");
                    }
                    ...
                    row.on("focusout", "input[datafield=temp_qty]", function () {
                        //if (item.qty_type == "Integer")
                        //    $(this).val(parseInt($(this).val()));
                        if ($(row).find("[id=itemUnit]").val() == "0") {
                            item.quantity = parseFloat(item.temp_qty);
                    ...
                    row.on("keydown", "input[datafield=temp_free_qty]", function (e) {
                        if (e.which == 9 || e.which == 13) {
                            ...
                            if (nextRow.length == 0) {
                                if (CONTEXT.ENABLE_SERIAL_NO_IN_POS) {
                                    row.find("input[datafield=serial_no]").focus().select();
                                } else {
                                    page.controls.txtItemSearch.selectedObject.focus();
                                }
                            } else {
                    ...
                    row.on("keydown", "input[datafield=serial_no]", function (e) {
                        if (e.which == 9 || e.which == 13) {
                            e.preventDefault();
                            page.controls.txtItemSearch.selectedObject.focus();
                        }
                    ...
                    row.on("keydown", "input[datafield=tofullfilled]", function (e) {
                        if (e.which == 9 || e.which == 13) {
                            e.preventDefault();
                            try {
                                if (item.qty_type == "Integer" && CheckDecimal(item.tofullfilled)) {
                                    throw "Incorrect Quantity Type";
                                }
                                if (item.qty_type == "Decimal" && !CheckDecimal(item.tofullfilled)) {
                                    throw "Incorrect Quantity Type";
                                }
                            }
                            catch (e) {
                                alert(e);
                                row.find("input[datafield=tofullfilled]").focus().select();
                            }
                        }
                    });
                    row.on("keydown", "input[datafield=todelivered]", function (e) {
                        if (e.which == 9 || e.which == 13) {
                            e.preventDefault();
                            try {
                                if (item.qty_type == "Integer" && CheckDecimal(item.todelivered)) {
                                    throw "Incorrect Quantity Type";
                                }
                                if (item.qty_type == "Decimal" && !CheckDecimal(item.todelivered)) {
                                    throw "Incorrect Quantity Type";
                                }
                            }
                            catch (e) {
                                alert(e);
                                row.find("input[datafield=todelivered]").focus().select();
                            }
                        }
                    });
                    var attrTemplate = [];
                    ...
                    row.on("click", "[datafield=sku_no]", function () {
                        ...
                        if (page.clickCount == 2) {
                            if (item.state_text == "Created" || item.state_text == "Ordered") {
                                page.clickCount = 0;
                                page.events.btnItemStock_click();
                            }
                        }
                    });
                    if (page.fullfillMode) {
                        item.fullfilled = (item.fullfilled) == null ? "0" : item.fullfilled;
                        $(row).find("[datafield=pendingfullfill]").find("div").html((parseFloat(item.temp_qty) + parseFloat(item.temp_free_qty)) - parseFloat(item.fullfilled));
                    }
                    if (page.deliveryMode) {
                        item.fullfilled = (item.fullfilled) == null ? "0" : item.fullfilled;
                        item.delivered = (item.delivered) == null ? "0" : item.delivered;
                        $(row).find("[datafield=pendingdelivery]").find("div").html(parseFloat(item.fullfilled)- parseFloat(item.delivered));
                    }
                    page.calculate_delivery_amount();
                    page.checkCrossButton();
                    calc_qty = (item.quantity == null || item.quantity == "") ? 0 : parseFloat(item.quantity);
                    calc_free_qty = (item.free_qty == null || item.free_qty == "") ? 0 : parseFloat(item.free_qty);
                    tot_qty = parseFloat(parseFloat(tot_qty) + parseFloat(calc_qty) + parseFloat(calc_free_qty)).toFixed(CONTEXT.COUNT_AFTER_POINTS);
                ...
                if(page.fullfillMode || page.deliveryMode || page.trayMode)
                    page.controls.grdSOItems.edit(true);
                $$("grdSOItems").selectedObject.find(".grid_header:last-child").hide();
                $$("grdSOItems").selectedObject.append("<div class='grid_bill_footer' style='background:white;padding:3px;padding-left:5px;font-size:12px;'>" + $$("grdSOItems").selectedObject.find(".grid_header:first-child").html() + "</div>");
                $$("grdSOItems").selectedObject.find(".grid_bill_footer div").html("");
                $$("grdSOItems").selectedObject.find(".grid_bill_footer div[datafield=item_name]").html("Total Qty : " + tot_qty);
            ...
            selectedReturnSOItems: function (data) {
                ...
                $$("grdReturnSOItems").setTemplate({
                    ...
                    columns: [
                        ...
                        { 'name': "Item No", 'rlabel': 'Item No', 'width': "0px", 'dataField': "item_no", editable: false, visible: false },
                        { 'name': "Item No", 'rlabel': 'Item No', 'width': "80px", 'dataField': "item_code", editable: false },
                        { 'name': "VAR", 'width': "60px", 'dataField': "var_no" },
                        { 'name': "SKU", 'rlabel': 'SKU', 'width': "60px", 'dataField': "sku_no", visible: CONTEXT.SHOW_SKU_COLUMN_IN_SALES },
                        { 'name': "Item Name", 'width': "250px;word-break: break-word;white-space: pre-wrap;", 'dataField': "item_name", editable: false, itemTemplate: "<div  id='prdItemName' style='height:auto;'></div>" },
                        ...
                        { 'name': "Rack No", 'width': "100px", 'dataField': "rack_no", visible: CONTEXT.ENABLE_RACK },
                        { 'name': "Serial No", 'rlabel': 'Serial No', 'width': "150px", 'dataField': "serial_no", editable: false, visible: CONTEXT.ENABLE_SERIAL_NO_IN_POS },
                        { 'name': "Attributes", 'rlabel': 'Attributes', 'dataField': "attributes", 'width': "400px", itemTemplate: "<div  id='Attributes'></div>", visible: false },
                ...
                page.controls.grdReturnSOItems.rowBound = function (row, item) {
                    ...
                    item.price = (item.price == null || isNaN(item.price)) ? 0 : item.price;
                    item.unit_identity = "0";
                    $(row).find("[datafield=unit_identity]").find("div").html(0);
                    ...
                    row.on("focusout", "input[datafield=ret_qty]", function (e) {
                        if (e.which == 9 || e.which == 13) {
                            e.preventDefault();
                            try {
                                if (item.qty_type == "Integer" && CheckDecimal(item.ret_qty)) {
                    ...
                    if (maxWidth < 100)
                        maxWidth = 100;
                    $($$("grdReturnSOItems").selectedObject).find("div[datafield=attributes]").width(maxWidth);
                    ...
                    if (item.attr_value2 != "" && item.attr_value2 != null && typeof item.attr_value2 != "undefined") {
                        itemTemplate.push("<br><span style=''>" + item.attr_type2 + ":" + item.attr_value2 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                    }
                    if (item.attr_value3 != "" && item.attr_value3 != null && typeof item.attr_value3 != "undefined") {
                        itemTemplate.push("<br><span style=''>" + item.attr_type3 + ":" + item.attr_value3 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                    }
                    if (item.attr_value4 != "" && item.attr_value4 != null && typeof item.attr_value4 != "undefined") {
                        itemTemplate.push("<br><span style=''>" + item.attr_type4 + ":" + item.attr_value4 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                    }
                    if (item.attr_value5 != "" && item.attr_value5 != null && typeof item.attr_value5 != "undefined") {
                        itemTemplate.push("<br><span style=''>" + item.attr_type5 + ":" + item.attr_value5 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                    }
                    if (item.attr_value6 != "" && item.attr_value6 != null && typeof item.attr_value6 != "undefined") {
                        itemTemplate.push("<br><span style=''>" + item.attr_type6 + ":" + item.attr_value6 + "</span><span style='padding-left:2px;padding-right:2px'>|</span>");
                    }
                    $(row).find("[id=prdItemName]").html(itemTemplate.join(""));
                }
                ...
                $$("grdReturnedSOItems").setTemplate({
                    ...
                    columns: [
                        ...
                        { 'name': "Bill No", 'width': "60px", 'dataField': "bill_no", editable: false },
                            { 'name': "Item No", 'rlabel': 'Item No', 'width': "60px", 'dataField': "item_no", editable: false },
            ...
            viewSOEvents: function () {

                $$("grdSOEvents").width("100%");
                $$("grdSOEvents").height("220px");
                $$("grdSOEvents").setTemplate({
                    selection: "Single",
                    paging: true, 
                    pageSize:50,
                    columns: [
                        { 'name': "Event ID", 'width': "80px", 'dataField': "event_id" },
                        { 'name': "Event Name", 'width': "150px", 'dataField': "event_name" },
                        { 'name': "Order Ref", 'width': "350px", 'dataField': "order_id" },
                        { 'name': "Item No", 'rlabel': 'Item No', 'width': "100px", 'dataField': "item_no", visible: false },
                        { 'name': "Var No", 'width': "80px", 'dataField': "var_no",visible:false },
                        { 'name': "Status", 'width': "80px", 'dataField': "status" },
                        //{ 'name': "", 'width': "auto", 'dataField': "event_id", itemTemplate: "<input type='button'id='idChangeEventState' class='grid-button' value='ChangeStatus' action='Change Status' style='background-size: contain;margin-right:10px' />" }
                        {
                            'name': "Action", 'width': "auto", 'dataField': "event_id", editable: false, itemTemplate: "<input type='button' title='View Item' id='eventOpen' class='grid-button' value='' action='Open' style='border:none;background-color:transparent;background-image:url(Open_file.png);background-size:contain;width:25px;height:25px;margin-right:10px' />" +
                                                                                                                "<input type='button' title='Confirm Cancel' id='eventConfirmCancel' class='grid-button' value='' action='ConfirmCancel' style='border:none;background-color:transparent;background-image:url(cancel.png);background-size:contain;width:25px;height:25px;margin-right:10px;margin-left:10px' />" +
                                                                                                                "<input type='button' title='Confirm Return' id='eventConfirmReturn' class='grid-button' value='' action='ConfirmReturn' style='border:none;background-color:transparent;background-image:url(Return-Purchase-icon.png);background-size:contain;width:25px;height:25px;margin-right:10px;margin-left:10px' />"
                        }
                    ]
                });
                page.controls.grdSOEvents.rowCommand = function (action, actionElement, rowId, row, rowData) {
                    if (action == "ChangeStatus") {
                        var evntData = {
                            event_id: rowData.event_id,
                            state_no: (rowData.state_no == "10" ? "30" : "10")
                        }
                        page.eventsAPI.putValue(evntData, function (data) {
                    ...
                    if (action == "Open") {
                        page.salesOrderAPI.searchValues("", "", " order_id in (select order_id from sal_order_t where order_ref_no like '" + rowData.order_id + "')", "", function (data) {
                            page.salesService.getSOItems(data[0].order_id, function (data) {
                                var fulfill = (rowData.event_name == "Order Cancel" || rowData.event_name == "Order Item Cancel") ? true : false;
                                if (fulfill) {
                                    page.controls.pnlViewOrderItemsPopup.open();
                                    page.controls.pnlViewOrderItemsPopup.title("View Order Items");
                                    page.controls.pnlViewOrderItemsPopup.rlabel("View Order Items");
                                    page.controls.pnlViewOrderItemsPopup.width("60%");
                                    page.controls.pnlViewOrderItemsPopup.height(400);
                                }
                                else {
                                    page.controls.pnlViewOrderItemsPopup.open();
                                    page.controls.pnlViewOrderItemsPopup.title("View Order Items");
                                    page.controls.pnlViewOrderItemsPopup.rlabel("View Order Items");
                                    page.controls.pnlViewOrderItemsPopup.width("75%");
                                    page.controls.pnlViewOrderItemsPopup.height(400);
                                }
                                page.view.selectedOrderItemView(data, fulfill);
                            });
                        });
                    }
                    if (action == "ConfirmCancel") {
                        //var evntData = {
                        //    event_id: page.controls.grdSales.selectedData()[0].event_id,
                        //    state_no: (page.controls.grdSales.selectedData()[0].state_no == "10" ? "30" : "10")
                        //}
                        page.salesOrderAPI.searchValues("", "", " sot.order_id in (select order_id from sal_order_t where sot.order_ref_no like '" + rowData.order_id + "')", "", function (data) {
                            page.selectedSO = data[0];
                            $$("ddlSOCustomer").selectedValue(page.selectedSO.cust_no);
                            $$("txtContactNo").value(page.selectedSO.contact_no);
                            $$("txtEmail").value(page.selectedSO.email);
                            $$("lblCustomerReward").html(page.selectedSO.points);
                            $$("txtDeliveryAddress").value(page.selectedSO.delivery_address);
                            $$("lblSOIBillNo").html(page.selectedSO.bill_no);
                            page.controls.lblSubTotal.value(page.selectedSO.sub_total);
                            page.controls.lblRndOff.value(page.selectedSO.round_off);
                            page.controls.lblTotal.value(page.selectedSO.total);
                            page.controls.lblDiscount.value(page.selectedSO.discount);
                            page.controls.lblTax.value(page.selectedSO.tax);
                            page.allowReturnPayment = false;
                            page.events.btnCancelSO_click();
                            //page.eventsAPI.putValue(evntData, function (data) {
                            //    alert("Status Updated successfully...!");
                            //    page.view.viewSOEvents();
                            //});
                    ...
                    if (action == "ConfirmReturn") {
                        ...
                        page.salesOrderAPI.searchValues("", "", " sot.order_id in (select order_id from sal_order_t where sot.order_ref_no like '" + rowData.order_id + "')", "", function (data) {
                            ...
                            page.controls.lblTax.value(page.selectedSO.tax);
                            page.allowReturnPayment = false;
                            page.events.btnCompleteReturn_click();
                            //page.eventsAPI.putValue(evntData, function (data) {
                            //    alert("Status Updated successfully...!");
                            //    page.view.viewSOEvents();
                            //});
                ...
                $$("grdSOEvents").beforeRowBound = function (row, item) {
                    if (item.state_no == "10") {
                        item.status = "Open";
                    } else if (item.state_no == "30") {
                        item.status = "Closed";
                    } else {
                        item.status =""
                    }
                }
                $$("grdSOEvents").rowBound = function (row, item) {
                    //$(row).find("[id=idChangeEventState]").click(function () {
                    //    var evntData = {
                    //        event_id: item.event_id,
                    //        state_no: (item.state_no == "10" ? "30" : "10")
                    //    }
                    //    page.eventsAPI.putValue(evntData, function (data) {
                    //        alert("Status Updated successfully...!");
                    //        page.view.viewSOEvents();
                    ...
                    //});
                    if (item.state_no == "30") {
                        $(row).find("[id=eventConfirmReturn]").hide();
                        $(row).find("[id=eventConfirmCancel]").hide();
                    }
                    else {
                        if (item.event_name == "Order Cancel" || item.event_name == "Order Item Cancel") {
                            $(row).find("[id=eventConfirmCancel]").show();
                            $(row).find("[id=eventConfirmReturn]").hide();
                        }
                        if (item.event_name == "Order Return" || item.event_name == "Order Item Return") {
                            $(row).find("[id=eventConfirmCancel]").hide();
                            $(row).find("[id=eventConfirmReturn]").show();
                        }
                    }
                }
                var state = "";
                if ($$("ddlSOEventStatus").selectedValue() == "Open") {
                    state = " state_no=10";
                } else if ($$("ddlSOEventStatus").selectedValue() == "Closed") {
                    state = " state_no=30";
                }
                var evntData = {
                    start_record: "",
                    end_record: "",
                    filter_expression: ""+state,
                    sort_expression:" event_id desc"
                }
                page.eventsAPI.searchValueCount(evntData.start_record, evntData.end_record, evntData.filter_expression, evntData.sort_expression, function (data) {
                    var totalRecord = data[0].cnt;
                    $$("grdSOEvents").dataBind({
                        getData: function (start, end, sortExpression, filterExpression, callback) {
                            page.eventsAPI.searchValue(start, end, evntData.filter_expression, evntData.sort_expression, function (data) {
                                callback(data, totalRecord);
                            });
                        }
                    });
            ...
            selectedPendingPayment: function (data) {
                ...
                $$("grdPendingPayment").height("280px");
                $$("grdPendingPayment").setTemplate({
                    selection: "Single",
                    columns: [
                        { 'name': "Order No", 'width': "80px", 'dataField': "order_no" },
                        { 'name': "Order Ref No", 'width': "110px", 'dataField': "order_ref_no" },
                        { 'name': "Bill No", 'rlabel': 'Bill No', 'width': "110px", 'dataField': "bill_code" },
                        { 'name': "Bill No", 'width': "80px", 'dataField': "bill_no", visible: false },
                        { 'name': "Bill Date", 'width': "100px", 'dataField': "bill_date" },
                        { 'name': "Bill Type", 'width': "120px", 'dataField': "bill_type" },
                        { 'name': "Total Amount", 'width': "120px", 'dataField': "total" },
                        { 'name': "Payment Date", 'width': "120px", 'dataField': "pay_date", visible: ($$("ddlBillView").selectedValue() == "Payment View") },
                        { 'name': "Paid", 'width': "90px", 'dataField': "total_paid_amount" },
                        { 'name': "Balance", 'width': "100px", 'dataField': "balance", visible: ($$("ddlBillView").selectedValue() != "Payment View") },
                        { 'name': "", 'width': "0px", 'dataField': "expense_amt" },
                        { 'name': "Amount Pay", 'width': "110px", 'dataField': "amount_pay", editable: true, visible: ($$("ddlBillView").selectedValue() != "Payment View") },
                        { 'name': "Pay Mode", 'width': "100px", 'dataField': "pay_mode", visible: ($$("ddlBillView").selectedValue() == "Payment View") },
                        ...
                        { 'name': "Bill No", 'width': "80px", 'dataField': "cust_name", visible: false },
                        { 'name': "Bill No", 'width': "80px", 'dataField': "contact_no", visible: false },
                        { 'name': "Payment ID", 'width': "80px", 'dataField': "payment_id", visible: true },
                    ]
                });
                //Handle Row Command
            ...
            selectedOrderItemView: function (data, fulfill) {
                $$("grdViewSOItems").width("100%");
                $$("grdViewSOItems").height("350px");
                $$("grdViewSOItems").setTemplate({
                    selection: "Single",
                    columns: [
                        { 'name': "Item Name", 'width': "150px", 'dataField': "item_name"},
                        { 'name': "Price", 'width': "120px", 'dataField': "price"},
                        { 'name': "Qty", 'width': "120px", 'dataField': "quantity"},
                        { 'name': "Unit", 'width': "100px", 'dataField': "unit" },
                        { 'name': "Fullfilled", 'width': "150px", 'dataField': "fullfilled",visible:fulfill },
                        { 'name': "Delivered", 'width': "150px", 'dataField': "delivered", visible: !fulfill },
                        { 'name': "Returned", 'width': "150px", 'dataField': "qty_returned", visible: !fulfill },
                        { 'name': "Total Price", 'width': "150px", 'dataField': "total_price"},
                    ]
                });
                page.controls.grdViewSOItems.dataBind(data);
            },
            ...
            UIState: function (state) {
                ...
                $$("txtSOCompany").disable(true);
                $$("txtShippingAddress").disable(true);
                $$("txtDeliveryAddress").disable(true);
                $$("txtContactNo").disable(true);
                ...
                if (CONTEXT.ENABLE_ADD_ADDITIONAL_ITEM) {
                    $$("btnUpdateItems").show();
                } else {
                    $$("btnUpdateItems").hide();
                }
                ...
                $$("btnEditOrderDate").hide();
                $$("btnSaveOrderDate").hide();
                $$("btnEditConfirmedDate").hide();
                $$("btnSaveConfirmedDate").hide();
                $$("btnEditFulfilledDate").hide();
                $$("btnSaveFulfilledDate").hide();
                $$("btnEditDeliveredDate").hide();
                $$("btnSaveDeliveredDate").hide();
                $$("btnEditInvoiceDate").hide();
                $$("btnSaveInvoiceDate").hide();
                $$("btnEditReturnedDate").hide();
                $$("btnSaveReturnedDate").hide();

                $$("pnlSOCancelledDate").hide();

                $$("btnPrintRequestForFulfill").hide();
                page.controls.btnPrintRequestForFulfill.selectedObject.prev().hide();
                page.removeItemsInGrid = false;
                page.checkCrossButton();
                $$("pnlTransactions").show();
                $$("pnlReplacementBill").hide();
                if (state == "Created") {
                    //Labels
                    ...
                    page.controls.btnReturnPOItem.selectedObject.prev().hide();
                    page.controls.btnCompleteReturnSO.selectedObject.prev().hide();
                    page.controls.btnBack.selectedObject.prev().hide();
                    page.controls.btnReCreateSO.selectedObject.next().hide();
                    $$("pnlBillSummary").show();

                    //Sales Executive
                    page.controls.ddlDeliveryBy.disable(false);
                    //$$("txtSOIBillDate").setDate($.datepicker.formatDate("dd-mm-yy", new Date()));
                    $$("txtSOIBillDate").setDate($.datepicker.formatDate("yy-mm-dd", new Date()));

                    //Expense Details
                    $$("txtExpenseName").disable(false);
                    $$("txtExpense").disable(false);
                    $$("txtSOCompany").disable(false);
                    $$("txtShippingAddress").disable(false);
                    $$("txtDeliveryAddress").disable(false);
                    $$("txtContactNo").disable(false);
                ...
                if (state == "Ordered") {
                    ...
                    page.controls.btnBack.selectedObject.prev().show();
                    ...
                    if (CONTEXT.ENABLE_PRINT_FULFILL_ITEMS) {
                        $$("btnPrintRequestForFulfill").show();
                        page.controls.btnPrintRequestForFulfill.selectedObject.prev().show();
                    } else {
                        $$("btnPrintRequestForFulfill").hide();
                        page.controls.btnPrintRequestForFulfill.selectedObject.prev().hide();
                    }
                ...
                if (state == "Confirmed") {
                    ...
                    $$("btnCompleteFullfill").show();
                    page.controls.btnCompleteFullfill.selectedObject.next().show();
                    $$("btnAddToFullfill").show();
                    page.controls.btnAddToFullfill.selectedObject.next().show();
                    $$("btnCompleteDelivery").show();
                    page.controls.btnCompleteDelivery.selectedObject.next().show();
                    ...
                    if (CONTEXT.ENABLE_MODULE_TRAY == false) {
                        ...
                        $$("btnAddToTray").show();
                        page.controls.btnAddToTray.selectedObject.next().show();
                        $$("btnReturnTray").show();
                        page.controls.btnReturnTray.selectedObject.next().hide();
                    }
                ...
                if (state == "Fulfilled") {
                    ...
                    //View the controls
                    $$("btnCompleteFullfill").hide();
                    page.controls.btnCompleteFullfill.selectedObject.next().hide();
                    ...
                    page.controls.btnReCreateSO.selectedObject.next().show();
                    page.controls.btnReturnPayInvoiceAmount.selectedObject.next().hide();
                    page.controls.btnInvoicePaySO.selectedObject.next().hide();
                    page.controls.btnRemoveSO.selectedObject.next().hide();
                    page.controls.btnInvoiceSO.selectedObject.next().show();
                ...
                if (state == "Delivered") {
                    ...
                    $$("btnAddToFullfill").hide();
                    page.controls.btnAddToFullfill.selectedObject.next().hide();
                    $$("btnCompleteDelivery").hide();
                    page.controls.btnCompleteDelivery.selectedObject.next().hide();
                    ...
                    page.controls.btnRemoveSO.selectedObject.next().hide();
                    page.controls.btnInvoiceSO.selectedObject.next().show();
                    page.controls.btnReturnPOItem.selectedObject.prev().show();
                    page.controls.btnCompleteReturnSO.selectedObject.prev().show();
                    page.controls.btnReCreateSO.selectedObject.next().hide();
                    page.controls.btnBack.selectedObject.prev().show();
                ...
                if (state == "Cancel") {
                    ...
                    page.controls.btnReturnPrintSO.selectedObject.prev().hide();
                    page.controls.btnInvoiceSO.selectedObject.next().show();
            ...
            selectedReplaceGrid: function (data) {
                $$("grdReplacementBill").width("100%");
                $$("grdReplacementBill").height("150px");
                $$("grdReplacementBill").setTemplate({
                    selection: "Single",
                    columns: [
                        { 'name': "Order Id", 'width': "150px", 'dataField': "order_id" },
                        { 'name': "State", 'width': "250px;word-break: break-word;white-space: pre-wrap;", 'dataField': "state_text" },
                        { 'name': "Reason", 'width': "500px;word-break: break-word;white-space: pre-wrap;", 'dataField': "reason" },
                        { 'name': "Action", 'width': "200px;", 'dataField': "order_is", itemTemplate: "<div  id='prdReplaceOrders' style='height:auto;'></div>" },
                    ]
                });
                page.controls.grdReplacementBill.rowCommand = function (action, actionElement, rowId, row, rowData) {
                    if (action == "ChangeToReplaced") {
                        if (confirm("Are Sure Want To Change This State As Replaced?")) {
                            var data = {
                                order_id: rowData.order_id,
                                status: "951",
                                state_text:"Replaced"
                            }
                            page.returnOrderAPI.putValue(rowData.order_id, data, function (data) {
                    ...
                    if (action == "ChangeToReplacement") {
                        if (confirm("Are Sure Want To Change This State As Replacement?")) {
                            var data = {
                                order_id: rowData.order_id,
                                status: "950",
                                state_text: "Request for Replacement"
                            }
                            page.returnOrderAPI.putValue(rowData.order_id, data, function (data) {
                                alert("Status Changed Successfully...!!!");
                                page.events.btnGetReplaceOrders_click();
                ...
                page.controls.grdReplacementBill.selectionChanged = function (row, item) {
                    page.returnOrderItemAPI.searchValues("", "", "order_id = " + item.order_id, "", function (data) {
                        page.view.selectedReplacementItems(data);
                    })
                }
                page.controls.grdReplacementBill.rowBound = function (row, item) {
                    var htmlTemplate = [];
                    if (item.status == "950") {
                        htmlTemplate.push("<input type='button' class='grid-button' value='' action='ChangeToReplaced' style='border: none;background-color: transparent;background-image: url(Open_file.png);background-size: contain;width: 25px;height: 25px;margin-right:10px' />");
                    }
                    if (item.status == "951") {
                        htmlTemplate.push("<input type='button' class='grid-button' value='' action='ChangeToReplacement' style='border: none;background-color: transparent;background-image: url(Open_file.png);background-size: contain;width: 25px;height: 25px;margin-right:10px' />");
                    }
                    $(row).find("[id=prdReplaceOrders]").html(htmlTemplate.join(""));
                }
                page.controls.grdReplacementBill.dataBind(data);
                page.view.selectedReplacementItems([]);
            },
            selectedReplacementItems: function (data) {
                $$("grdReplacementBillItems").width("100%");
                $$("grdReplacementBillItems").height("150px");
                $$("grdReplacementBillItems").setTemplate({
                    selection: "Single",
                    columns: [
                        { 'name': "Image", 'width': "200px", 'dataField': "item_image_name", itemTemplate: "<div  id='prdItemName' style='height:auto;'></div>" },
                        { 'name': "Item Name", 'width': "250px;word-break: break-word;white-space: pre-wrap;", 'dataField': "item_name" },
                        { 'name': "Model Name", 'width': "250px;word-break: break-word;white-space: pre-wrap;", 'dataField': "model_name" },
                        { 'name': "Quantity", 'width': "150px;word-break: break-word;white-space: pre-wrap;", 'dataField': "quantity" },
                        { 'name': "Unit", 'width': "150px;word-break: break-word;white-space: pre-wrap;", 'dataField': "unit" },
                        { 'name': "SKU No", 'width': "150px;word-break: break-word;white-space: pre-wrap;", 'dataField': "sku_no" },
                    ]
                });
                page.controls.grdReplacementBillItems.rowBound = function (row, item) {
                    var htmlTemplate = [];
                    htmlTemplate.push('<img src=' + CONTEXT.ENABLE_IMAGE_DOWNLOAD_PATH + item.item_id + '/' + item.image_name + ' width="100%;" />')
                    $(row).find("[id=prdItemName]").html(htmlTemplate.join(""));
                };
                page.controls.grdReplacementBillItems.dataBind(data);
            },
        }
        //QR Code Scan
        ...
        page.events.btnItemStock_click = function () {
            var item = $$("grdSOItems").selectedData()[0];
            ...
            page.controls.grdItemStock.rowBound = function (row, item) {
                var data = page.controls.grdSOItems.selectedData()[0];
                var attrTemplate = [];
                if (item.var_stock_attr_key != null) {
                    if (item.var_stock_attr_key.split(",").length - 1 >= 0) {
                        attrTemplate.push("<input type='text' dataField='attr_value1' style='width:80px; background: transparent;border:none;' placeholder='" + getAttributeName(item.var_stock_attr_key.split(",")[0]) + "' >");
        ...
        page.events.btnSaveItemStock_click = function () {
            $(page.controls.grdSOItems.selectedRows()).each(function (i, row) {
                var billItem = page.controls.grdSOItems.getRowData(row);
        ...
        page.events.btnbtnEditOrderDate_click = function () {
            $$("lblSOOrderedDate").disable(false);
            $$("btnEditOrderDate").hide();
            $$("btnSaveOrderDate").show();
        }
        page.events.btnbtnSaveOrderDate_click = function () {
            $$("lblSOOrderedDate").disable(true);
            $$("btnEditOrderDate").show();
            $$("btnSaveOrderDate").hide();
            var data = {
                ordered_date: dbDateTime(page.controls.lblSOOrderedDate.getDate()),
                order_id: page.selectedSO.order_id,
            }
            page.salesOrderAPI.putValue(page.selectedSO.order_id, data, function (temp) {
                page.loadSalesOrder(page.selectedSO.order_id, function () {});
            });
        }
        page.events.btnbtnEditConfirmedDate_click = function () {
            $$("lblSOConfirmedDate").disable(false);
            $$("btnEditConfirmedDate").hide();
            $$("btnSaveConfirmedDate").show();
        }
        page.events.btnbtnSaveConfirmedDate_click = function () {
            $$("lblSOConfirmedDate").disable(true);
            $$("btnEditConfirmedDate").show();
            $$("btnSaveConfirmedDate").hide();
            var data = {
                confirm_date: dbDateTime(page.controls.lblSOConfirmedDate.getDate()),
                order_id: page.selectedSO.order_id,
            }
            page.salesOrderAPI.putValue(page.selectedSO.order_id, data, function (temp) {
                page.loadSalesOrder(page.selectedSO.order_id, function () { });
        ...
        page.events.btnbtnEditFulfilledDate_click = function () {
            $$("lblSOFulfilledDate").disable(false);
            $$("btnEditFulfilledDate").hide();
            $$("btnSaveFulfilledDate").show();
        }
        page.events.btnbtnSaveFulfilledDate_click = function () {
            $$("lblSOFulfilledDate").disable(true);
            $$("btnEditFulfilledDate").show();
            $$("btnSaveFulfilledDate").hide();
            var data = {
                fulfilled_date: dbDateTime(page.controls.lblSOFulfilledDate.getDate()),
                order_id: page.selectedSO.order_id,
            }
            page.salesOrderAPI.putValue(page.selectedSO.order_id, data, function (temp) {
                page.loadSalesOrder(page.selectedSO.order_id, function () { });
        ...
        page.events.btnbtnEditDeliveredDate_click = function () {
            $$("lblSODeliveredDate").disable(false);
            $$("btnEditDeliveredDate").hide();
            $$("btnSaveDeliveredDate").show();
        }
        page.events.btnbtnSaveDeliveredDate_click = function () {
            $$("lblSODeliveredDate").disable(true);
            $$("btnEditDeliveredDate").show();
            $$("btnSaveDeliveredDate").hide();
            var data = {
                delivered_date: dbDateTime(page.controls.lblSODeliveredDate.getDate()),
                order_id: page.selectedSO.order_id,
            }
            page.salesOrderAPI.putValue(page.selectedSO.order_id, data, function (temp) {
                page.loadSalesOrder(page.selectedSO.order_id, function () { });
        ...
        page.events.btnbtnEditInvoiceDate_click = function () {
            $$("lblSOInvoiceDate").disable(false);
            $$("btnEditInvoiceDate").hide();
            $$("btnSaveInvoiceDate").show();
        }
        page.events.btnbtnSaveInvoiceDate_click = function () {
            $$("lblSOInvoiceDate").disable(true);
            $$("btnEditInvoiceDate").show();
            $$("btnSaveInvoiceDate").hide();
            var data = {
                invoice_paid_date: dbDateTime(page.controls.lblSOInvoiceDate.getDate()),
                order_id: page.selectedSO.order_id,
            }
            page.salesOrderAPI.putValue(page.selectedSO.order_id, data, function (temp) {
                page.loadSalesOrder(page.selectedSO.order_id, function () { });
        ...
        page.events.btnbtnEditReturnedDate_click = function () {
            $$("lblSOReturnedDate").disable(false);
            $$("btnEditReturnedDate").hide();
            $$("btnSaveReturnedDate").show();
        }
        page.events.btnbtnSaveReturnedDate_click = function () {
            $$("lblSOReturnedDate").disable(true);
            $$("btnEditReturnedDate").show();
            $$("btnSaveReturnedDate").hide();
            var data = {
                returned_date: dbDateTime(page.controls.lblSOReturnedDate.getDate()),
                order_id: page.selectedSO.order_id,
            }
            page.salesOrderAPI.putValue(page.selectedSO.order_id, data, function (temp) {
                page.loadSalesOrder(page.selectedSO.order_id, function () { });
        ...
        page.events.btnUpdateItems_click = function () {
            try {
                $(page.controls.grdSOItems.allData()).each(function (i, item) {
                    if (item.quantity == "" || item.quantity == null || typeof item.quantity == "undefined")
                        item.quantity = 0;
                    ...
                    if ((parseFloat(item.quantity) + parseFloat(item.free_qty)) < parseFloat(item.fullfilled))
                        throw "Item Quantity Cannot Be Less Than The Fulfilled Quantity For Item " + item.item_name;
                                ...
                                page.billService.removeAllSODiscount(page.selectedSO.order_id, function () {
                                    page.billService.reinsertSODisount(page.selectedSO.order_id, 0, page.discount, function () {
                                        ...
                                        page.stockAPI.insertBill(newBill, function (data) {
                                            page.currentBillNo = data.bill_no;
                                            page.loadSalesOrder(page.selectedSO.order_id);
                                            if (page.selectedSO.state_text == "Confirmed") {
                                                var newBill = {
                                                    bill_no: page.controls.lblSOIBillNo.html(),
                                                    bill_type: "Sale",
                                                    state_no: 200, //200 is sales
                                                ...
                                                if (CONTEXT.ENABLE_FINFACTS_MODULES == true) {
                                                    ...
                                                    var data1 = {
                                                        ...
                                                        per_id: CONTEXT.FINFACTS_CURRENT_PERIOD,
                                                        jrn_date: $.datepicker.formatDate("dd-mm-yy", new Date()),
                                                        description: "Sales Order - " + $$("lblTempSONo").html(),
                                                        sales_with_out_tax: parseFloat(s_with_tax).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                                                        tax_amt: parseFloat(page.controls.lblTax.value()).toFixed(CONTEXT.COUNT_AFTER_POINTS),
                                                        buying_cost: buying_cost,
                                                        round_off: $$("lblRndOff").value(),
                                                    ...
                                                    $$("msgPanel").flash("Updating Finfacts...");
                                                    page.finfactsEntry.creditSales(data1, function (response) {
                                                        if (CONTEXT.ENABLE_BILL_EXPENSE_MODULES) {
                                                            var expenseData1 = {
                                                                per_id: CONTEXT.FINFACTS_CURRENT_PERIOD,
        ...
        page.events.btnGetReplaceOrders_click = function () {
            page.returnOrderAPI.searchValues("", "", "rot.sal_order_id = "+page.selectedSO.order_id, "", function (data) {
                page.view.selectedReplaceGrid(data);
            })
        }
    });
}
    ...
    if (typeof control.controlName === "undefined") { //Do the intitialisation only one time.
        ...
        control.events = {
            ...
            page_load: function () {
                ...
                (CONTEXT.SHOW_CUSTOMER_COMPANY_IN_SALES) ? $$("pnlCustomerCompany").show() : $$("pnlCustomerCompany").hide();
                (CONTEXT.ENABLE_SALES_EXECUTIVE) ? $$("pnlSalesExecutive").show() : $$("pnlSalesExecutive").hide();
                (CONTEXT.ENABLE_CUST_GST) ? $$("pnlCustomerGst").show() : $$("pnlCustomerGst").hide();
            },
